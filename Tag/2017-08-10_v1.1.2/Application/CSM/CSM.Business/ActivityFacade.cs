    ...
    public class ActivityFacade : BaseFacade, IActivityFacade
    {
        ...
        public ServiceRequestSaveCarResult InsertActivityLogToCAR(AuditLogEntity auditLog, ServiceRequestForInsertLogEntity entity)
        {
            ...
            try
            {
                ...
                {
                    string accNo = entity.AccountNo.ToUpperInvariant();
                    if (accNo == "DUMMY")
                        accountNo = string.Format(CultureInfo.InvariantCulture, "{0}-{1}", accNo, entity.CustomerId);
                    else if (accNo == "NA")
                    {
                        //accountNo = string.Format(CultureInfo.InvariantCulture, "{0}-{1}", accNo, entity.ContactId);
                        accountNo = null;
                    }
                }
        ...
        {
            var activities = this.GetActivityLogList(auditLog, searchFilter);

            if (activities != null && activities.Any())
            {
                return activities.Select(x => new ServiceRequestActivityResult
                {
