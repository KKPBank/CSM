...
namespace CSM.Data.DataAccess
{
    public class CommPoolDataAccess : ICommPoolDataAccess
    {
        ...
        public bool AddMailContent(List<CommunicationPoolEntity> mailList, ref int refNumOfSR, ref int refNumOfFax,
                ...
                using (var transaction = _context.Database.BeginTransaction(IsolationLevel.ReadCommitted))
                {
                    ...
                    try
                    {
                        if (mailList != null && mailList.Count > 0)
                        {
                            ...
                            foreach (CommunicationPoolEntity mail in mailList)
                            {
                                ...
                                {
                                    int pos = mail.SenderAddress.IndexOf('@');
                                    createUserName = pos != -1 ? mail.SenderAddress.Substring(0, pos) : string.Empty;
                    ...
                    try
                    {
                        ...
                        if (jobAttachments.Any())
                        {
                            ...
                            foreach (var jobAttach in jobAttachments)
                            {
                                ...
                                var srFileNameUrl = ApplicationHelpers.GenerateFileName(srFolder, Path.GetExtension(jobFileName), seqNo);
                                ...
                                if (fileInfo != null && fileInfo.Length > 0)
                                {
                                    if (StreamDataHelpers.TryToCopy(jobFilePath, srFilePath))
                                    {
                                        ...
                                        var srAttachment = new SrAttachmentEntity
                                        {
                                            ...
                                            DocumentType = new List<AttachmentTypeEntity>(),
                                            CreateDate = DateTimeHelpers.FormatDateTime(DateTime.Now, Constants.DateTimeFormat.DefaultFullDateTime),
                                            IsEditable = true,
