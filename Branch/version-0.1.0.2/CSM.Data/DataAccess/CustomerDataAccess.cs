...
namespace CSM.Data.DataAccess
{
    public class CustomerDataAccess : ICustomerDataAccess
    {
        ...
        public IEnumerable<CustomerEntity> GetCustomerList(CustomerSearchFilter searchFilter)
        {
            ...
            sortList.Add("Product", " acc.PRODUCT ");
        ...
        public ExistingProductEntity GetExistingProductDetail(ExistingProductSearchFilter searchFilter)
        {
            ...
            productDeatil.AccountAddressList = queryAddress.Any() ? queryAddress.ToList() : null;
        ...
        public List<CustomerContactEntity> GetContactRelationshipList(int contactId, int currentCustomerId)
        {
            ...
            var query = from cc in _context.TB_M_CUSTOMER_CONTACT
                        ...
                        select new CustomerContactEntity
                        {
                            ...
                            CustomerFullNameThaiEng = cust != null ?
                                (!string.IsNullOrEmpty(cust.FIRST_NAME_TH) ? (cust.FIRST_NAME_TH + " " + cust.LAST_NAME_TH)
                                : (cust.FIRST_NAME_EN + " " + cust.LAST_NAME_EN))
                                : "",
                            CustomerId = cc.CUSTOMER_ID
                        };
        ...
        {
            var query = from acc in _context.TB_M_ACCOUNT.AsNoTracking()
                        where !string.IsNullOrEmpty(acc.PRODUCT)
                        && (string.IsNullOrEmpty(searchTerm) || acc.PRODUCT.Contains(searchTerm))
                        select new AccountEntity
                        {
                            Product = acc.PRODUCT
                        };
        ...
        public List<AccountEntity> GetAccountGradeByName(string searchTerm, int pageSize, int pageNum)
        {
            return GetAccountGradeQueryByName(searchTerm).OrderBy(x => x.Grade)
                .Skip(pageSize * (pageNum - 1))
                .Take(pageSize)
                .ToList();
        }
        ...
        // And the total count of records
        public int GetAccountGradeCountByName(string searchTerm)
        {
            return GetAccountGradeQueryByName(searchTerm).Count();
        }
        private IQueryable<AccountEntity> GetAccountGradeQueryByName(string searchTerm)
        {
            var query = from acc in _context.TB_M_ACCOUNT.AsNoTracking()
