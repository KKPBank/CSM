...
namespace CSM.Data.DataAccess
{
    public class ServiceRequestDataAccess : IServiceRequestDataAccess
    {
            ...
            try
            {
                ...
                if (timeToCheck < checkStartTime || timeToCheck > checkEndTime)
                {
                    if (timeToCheck >= checkEndTime && timeToCheck <= 2359)
                        currentSla = new DateTime(currentSla.Year, currentSla.Month, currentSla.Day + 1, startTimeHour, startTimeMin, 0);
                    else
                ...
                {
                    if (timeToCheck >= checkEndTime && timeToCheck <= 2359)
                        currentDate = new DateTime(currentDate.Year, currentDate.Month, currentDate.Day + 1, startTimeHour, startTimeMin, 0);
                    else
        ...
        public IQueryable<AccountAddressSearchResult> SetAccountAddressListSort(
            IQueryable<AccountAddressSearchResult> addressList, AccountAddressSearchFilter searchFilter)
        {
        ...
        public IQueryable<ServiceRequestCustomerSearchResult> SetCustomerListSort(
            IQueryable<ServiceRequestCustomerSearchResult> customerList, CustomerSearchFilter searchFilter)
        {
            if (searchFilter.SortOrder.ToUpper().Equals("ASC"))
            {
                switch (searchFilter.SortField.ToUpper())
        ...
        public IEnumerable<ServiceRequestAccountSearchResult> SearchAccount(ContractSearchFilter searchFilter)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.ProductName))
                query = query.Where(q => q.PRODUCT == searchFilter.ProductName);
        ...
        public IQueryable<ServiceRequestAccountSearchResult> SetContractListSort(
            IQueryable<ServiceRequestAccountSearchResult> contractList, ContractSearchFilter searchFilter)
        {
            if (searchFilter.SortOrder.ToUpper().Equals("ASC"))
            {
                switch (searchFilter.SortField.ToUpper())
        ...
        public IEnumerable<ServiceRequestContactSearchResult> SearchContact(CustomerContactSearchFilter searchFilter)
        {
            ...
            var resultQuery = query.Select(q => new ServiceRequestContactSearchResult()
            {
                ...
                TitleTh = q.TB_M_CONTACT.TB_M_TITLE.TITLE_NAME, //TitleTh = q.TB_M_CONTACT.TB_M_TITLE.TITLE_TH,
        ...
        public IQueryable<ServiceRequestContactSearchResult> SetCustomerContractListSort(
            IQueryable<ServiceRequestContactSearchResult> customerContractList, CustomerContactSearchFilter searchFilter)
        {
            if (searchFilter.SortOrder.ToUpper().Equals("ASC"))
            {
                switch (searchFilter.SortField.ToUpper())
...
//                            }).OrderBy(p => p.PhoneId).ToList(),
        ...
        public IQueryable<ServiceRequestActivityResult> GetSrActivityQueryable(bool isSrOnly, int? srId, int? customerId)
        {
            return (from sa in _context.TB_T_SR_ACTIVITY
                    ...
                    select new ServiceRequestActivityResult()
                    {
                        ...
                        EmailBody = sa.SR_EMAIL_TEMPLATE_ID.HasValue ? sa.SR_ACTIVITY_EMAIL_BODY : "",
        ...
        public bool CheckDuplicateDocumentFilename(AttachmentEntity attach)
        {
            ...
            {
                return
                    _context.TB_T_SR_ATTACHMENT.Any(i => i.SR_ID == attach.SrId && i.SR_ATTACHMENT_NAME == attach.Name);
            }
            else
            {
