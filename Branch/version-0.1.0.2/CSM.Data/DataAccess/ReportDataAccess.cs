    ...
    public class ReportDataAccess : IReportDataAccess
    {
        ...
        public IList<ExportSREntity> GetExportSR(ExportSRSearchFilter searchFilter)
        {
            ...
            var query = (from sr in _context.TB_T_SR.AsNoTracking()
                         ...
                         && (string.IsNullOrEmpty(searchFilter.Subject) || sr.SR_SUBJECT.ToUpper().Contains(searchFilter.Subject.ToUpper()))
                         && (string.IsNullOrEmpty(searchFilter.Description) || sr.SR_REMARK.ToUpper().Contains(searchFilter.Description.ToUpper()))
