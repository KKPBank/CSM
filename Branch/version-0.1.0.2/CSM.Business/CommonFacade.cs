...
namespace CSM.Business
{
    public class CommonFacade : ICommonFacade
    {
        ...
        public List<MenuEntity> GetCacheCustomerTab(string selectedTab)
        {
            // Check the cache
        ...
        public bool VerifyServiceRequest<T>(dynamic header)
        {
            ...
            if (typeof(T) == typeof(Header))
            {
                try
                {
                    query = from x in doc.Descendants("service")
                            where x.Element("service_name").Value.ToUpper(CultureInfo.InvariantCulture) == header.service_name.ToUpper(CultureInfo.InvariantCulture) &&
                                //x.Element("reference_no").Value == referenceNo &&
                                x.Element("system_code").Value.ToUpper(CultureInfo.InvariantCulture) == header.system_code.ToUpper(CultureInfo.InvariantCulture) &&
                                x.Element("username").Value.ToUpper(CultureInfo.InvariantCulture) == header.user_name.ToUpper(CultureInfo.InvariantCulture) &&
            ...
            Logger.Info(noError
                ...
                : _logMsg.Clear().SetPrefixMsg("Verify Request Header").ToSuccessLogString());
