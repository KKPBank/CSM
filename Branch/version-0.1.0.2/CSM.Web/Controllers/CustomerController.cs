...
namespace CSM.Web.Controllers
{
    ...
    public class CustomerController : BaseController
    {
        ...
        public ActionResult Search(string skip = "0")
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(this.CallId) && !skip.Equals("1"))
                {
                    ...
                    //custVM.SearchFilter.CardNo = callInfo.CardNoDisplay;
                    ...
                    if (custVM.SearchFilter.TotalRecords == 1)
                    {
                        var customer = custVM.CustomerList.First();
                        if (customer != null)
                        {                           
                            return CustomerNote(customer.CustomerId);
                        }
                    }
                }
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (isSuccess)
                    {
                        ...
                        return RedirectToAction("CustomerNote", "Customer");
                    }
        ...
        public ActionResult CustomerNote(int? customerId = null)
        {
            try
            {
        ...
        [HttpGet]
        [CheckUserRole(ScreenCode.ViewCustomerNote)]
        public ActionResult CustomerNote()
        {
            try
            {
                int? customerId = null;
        ...
        [HttpGet]
        public JsonResult SearchByGrade(string searchTerm, int pageSize, int pageNum)
        {
            _customerFacade = new CustomerFacade();
            List<AccountEntity> accounts = _customerFacade.GetAccountGradeByName(searchTerm, pageSize, pageNum);
            int total = _customerFacade.GetAccountGradeCountByName(searchTerm);
        ...
        private CustomerInfoViewModel MappingCustomerInfoView(int customerId)
        {
            _customerFacade = new CustomerFacade();
            // CustomerInfo
            CustomerEntity customerEntity = _customerFacade.GetCustomerByID(customerId);
