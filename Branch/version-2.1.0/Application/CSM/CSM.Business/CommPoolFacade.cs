    ...
    public sealed class CommPoolFacade : ICommPoolFacade
    {
        ...
        public JobTaskResult AddMailContent(string hostname, int port, bool useSsl, PoolEntity pool)
        {
            ...
            try
            {
                ...
                bool success = _commPoolDataAccess.AddMailContent(mailList, ref refNumOfSR, ref refNumOfFax, ref refNumOfWeb, ref refNumOfEmail, ref refNumOfKKAM);
                ...
                taskResult.NumOfKKAM = refNumOfKKAM;
                                ...
                                if (entity.ContentDisposition != null)
                                {
                                    ...
                                    if (entity.Body != null && entity.Body is MIME_b_SinglepartBase)
                                    {
                                        ...
                                        if (bytes != null && bytes.Length > 0)
                                        {
                                            ...
                                            {
                                                if (bytes.Length <= GetInlineContentLimit())
                                                {
                                                    ImageToBase64String(mail, contentId, contentType, bytes);
                                                }
                                                else
                                                {
                                                    runInlineFileName++;
                                                    InlineImageToAttach(mail, runInlineFileName, contentType, bytes);
                                                }
                                            }
                                        }
        ...
        private void ImageToBase64String(CommunicationPoolEntity mail, string contentId, string contentType, byte[] bytes)
        {
        ...
        private void InlineImageToAttach(CommunicationPoolEntity mail, int runFileName, string contentType, byte[] bytes)
        {
            string fileName;
            Logger.Info(_logMsg.Clear().SetPrefixMsg("Convert Inline to Attachment").Add("GetInlineContentLimit", GetInlineContentLimit())
                                         .Add("bytes.Length", bytes.Length).ToOutputLogString());
            string[] contentTypeSplit = contentType.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries);
            string ext = "jpg";
            if (contentTypeSplit != null && contentTypeSplit.Length > 1)
            {
                ext = contentTypeSplit[1];
            }
            fileName = $"InlineImage-{runFileName.ToString("00")}.{ext}";
            var attachment = new AttachmentEntity();
            attachment.ContentType = contentType;
            attachment.ByteArray = bytes;
            attachment.Filename = fileName;
        ...
        private int GetInlineContentLimit()
        {
            _commonFacade = new CommonFacade();
            ParameterEntity param = _commonFacade.GetParamByName(Constants.ParameterName.InlineContentLimit);
            return param != null ? param.ParamValue.ToNullable<int>().Value : 2048;
        }
