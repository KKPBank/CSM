...
namespace CSM.Data.DataAccess
{
    public class CustomerDataAccess : ICustomerDataAccess
    {
        ...
        public IEnumerable<RelationshipEntity> GetAllRelationships(RelationshipSearchFilter searchFilter)
        {
            var query = from rs in _context.TB_M_RELATIONSHIP.AsNoTracking()
                        ...
                        select new RelationshipEntity
                        {
                            ...
                            CbsRelationshipCode = rs.CBS_RELATIONSHIP_CODE,
                        };
        ...
        public bool IsDuplicateRelationship(RelationshipEntity relationEntity)
        {
            ...
            if (!string.IsNullOrWhiteSpace(relationEntity.CbsRelationshipCode))
            {
                if (cnt == 0)
                {
                    cnt = _context.TB_M_RELATIONSHIP.Where(
                            x => x.CBS_RELATIONSHIP_CODE == relationEntity.CbsRelationshipCode
                            && x.RELATIONSHIP_ID != relationEntity.RelationshipId
                        ).Count();
                }
            }
        ...
        public bool SaveContactRelation(RelationshipEntity relationEntity)
        {
            ...
            try
            {
                ...
                if (relationEntity.RelationshipId == 0)
                {
                    ...
                    relationship.CBS_RELATIONSHIP_CODE = relationEntity.CbsRelationshipCode;
        ...
        //{
        //    int? customerType = searchFilter.CustomerType;
        ...
        //                    } : null),
        //                    CustomerType = c.TYPE,
        //                    CbsCardType = (from ct in _context.TB_M_CARD_TYPE
        //                                   where ct.CARD_TYPE_ID == c.CARD_TYPE_ID
        //                                   select new CbsCardTypeEntity
        //                                   {
        //                                       CardTypeID = ct.CARD_TYPE_ID,
        //                                       CardTypeCode = ct.CARD_TYPE_CODE,
        //                                       CardTypeName = ct.CARD_TYPE_NAME
        //                                   }).FirstOrDefault(),
        ...
        //        if (status == 1)
        //        {
        //            query = query.Where(x => x.Account.AccountStatus == Constants.AccountStatus.Active);
        //        }
        //        else if (status == 0)
        //        {
        //            query = query.Where(x => x.Account.AccountStatus != Constants.AccountStatus.Active);
        ...
        {
            var query = _context.TB_M_CUSTOMER.AsQueryable();
            query = query.Where(a => a.TYPE != Constants.CustomerType.Customer);  //ค้นหาจาก Postpect กับ Enployee

            if (!string.IsNullOrEmpty(searchFilter.FirstName))
            {
                var originalKeyword = searchFilter.FirstName.Trim();
                if (searchFilter.ExactFirstName == false)
                {
                    query = query.Where(q => q.FIRST_NAME_TH.Contains(originalKeyword) || q.FIRST_NAME_EN.Contains(originalKeyword));
                }
                else
                {
                    query = query.Where(q => q.FIRST_NAME_TH == originalKeyword || q.FIRST_NAME_EN == originalKeyword);
                }
            }

            if (!string.IsNullOrEmpty(searchFilter.LastName))
            {
                var originalKeyword = searchFilter.LastName.Trim();
                if (searchFilter.ExactLastName == false)
                {
                    query = query.Where(q => q.LAST_NAME_TH.Contains(originalKeyword) || q.LAST_NAME_EN.Contains(originalKeyword));
                }
                else
                {
                    query = query.Where(q => q.LAST_NAME_TH == originalKeyword || q.LAST_NAME_EN == originalKeyword);
                }
            }

            //Filter : Account No
            if (!string.IsNullOrEmpty(searchFilter.AccountNo) && searchFilter.AccountNo.ToUpper() != "DUMMY")
            {
                query = query.Where(q => q.TB_M_ACCOUNT.Any(a => a.ACCOUNT_NO == searchFilter.AccountNo));
            }
            else if(!string.IsNullOrEmpty(searchFilter.AccountNo) && searchFilter.AccountNo.ToUpper() == "DUMMY")
            {
                //ถ้าใส่เลขบัญชีเป็น DUMMY ก็ไม่ต้องหาในฐานข้อมูล
                query = query.Where(q => 1 == 2);
            }

            //Filter : PhoneNo/Email
            if (!string.IsNullOrEmpty(searchFilter.PhoneNo))
            {
                if (searchFilter.ExactPhoneNo == true)
                {
                    query = query.Where(q => q.TB_M_PHONE.Any(p => p.PHONE_NO == searchFilter.PhoneNo) || (q.EMAIL == searchFilter.PhoneNo));
                }
                else
                {
                    query = query.Where(q => q.TB_M_PHONE.Any(p => p.PHONE_NO.Contains(searchFilter.PhoneNo)) || (q.EMAIL.Contains(searchFilter.PhoneNo)));
                }
            }
            ...
            if (!string.IsNullOrEmpty(searchFilter.CardNo))
            {
                query = query.Where(q => q.CARD_NO == searchFilter.CardNo);
            }

            //Filter : Registration  / Car No
            if (!string.IsNullOrEmpty(searchFilter.Registration))
            {
                query = query.Where(q => q.TB_M_ACCOUNT.Any(a => a.CAR_NO == searchFilter.Registration));
            }

            //Filter : Customer Type
            if ((searchFilter.CustomerType ?? 0) > 0)
            {
                query = query.Where(q => q.TYPE == searchFilter.CustomerType);
            }

            var results = (from c in query
                           from tiTh in _context.TB_M_TITLE.Where(th => th.TITLE_ID == c.TITLE_TH_ID).DefaultIfEmpty()
                           from tiEn in _context.TB_M_TITLE.Where(en => en.TITLE_ID == c.TITLE_EN_ID).DefaultIfEmpty()
                               //from sbt in _context.TB_M_SUBSCRIPT_TYPE.Where(sb => sb.SUBSCRIPT_TYPE_ID == c.SUBSCRIPT_TYPE_ID).DefaultIfEmpty()
                               //from ct in _context.TB_M_CUST_TYPE.Where(ct => ct.CUSTOMER_CATEGORY_VALUE == c.TB_M_SUBSCRIPT_TYPE.CUST_TYPE_GROUP).DefaultIfEmpty()
                           from u in _context.TB_R_USER.Where(x => x.USER_ID == c.EMPLOYEE_ID).DefaultIfEmpty()
                           select new CustomerEntity
                           {
                               CustomerId = c.CUSTOMER_ID,
                               CustomerNumber = 0,
                               TitleThai = new TitleEntity { TitleId = tiTh.TITLE_ID, TitleName = tiTh.TITLE_NAME ?? "" },
                               TitleEnglish = new TitleEntity { TitleId = tiEn.TITLE_ID, TitleName = tiEn.TITLE_NAME ?? "" },
                               FirstNameThai = c.FIRST_NAME_TH,
                               FirstNameEnglish = c.FIRST_NAME_EN,
                               LastNameThai = c.LAST_NAME_TH,
                               LastNameEnglish = c.LAST_NAME_EN,
                               FirstNameThaiEng = !string.IsNullOrEmpty(c.FIRST_NAME_TH) ? c.FIRST_NAME_TH : "",
                               LastNameThaiEng = !string.IsNullOrEmpty(c.LAST_NAME_TH) ? c.LAST_NAME_TH : "",
                               CardNo = c.CARD_NO,
                               IDTypeCode = c.CARD_TYPE_CODE, //sbt.SUBSCRIPT_TYPE_NAME ?? "",
                               CbsCardType = (from ct in _context.TB_M_CARD_TYPE
                                              where ct.CARD_TYPE_ID == c.CARD_TYPE_ID
                                              select new CbsCardTypeEntity
                                              {
                               ...
                               CustomerType = c.TYPE ?? 0,
                               CustomerCategory = "",//_context.TB_M_CUST_TYPE.FirstOrDefault().LONG_DESC_THAI,
                               AccountNo = "",
                               ...
                               CustomerWebserviceType = Constants.CustomerWebServiceType.CustomerPostpect,
                               PhoneList = _context.TB_M_PHONE.Where(x => x.CUSTOMER_ID == c.CUSTOMER_ID).DefaultIfEmpty()
                                            .Take(3).Select(x => new PhoneEntity()
                                            {
                                                PhoneNo = x.PHONE_NO,
                                                PhoneTypeName = x.TB_M_PHONE_TYPE.PHONE_TYPE_NAME
                                            }).ToList(),
                               //PhoneList = c.TB_M_PHONE.Any() ? c.TB_M_PHONE.Take(3)
                               //             .Select(x => new PhoneEntity()
                               //             {
                               //                 PhoneNo = x.PHONE_NO,
                               //                 PhoneTypeName = x.TB_M_PHONE_TYPE != null ? x.TB_M_PHONE_TYPE.PHONE_TYPE_NAME : string.Empty
                               //             }).ToList() : new List<PhoneEntity>()
                               EmployeeId = c.EMPLOYEE_ID,
                               EmployeeCode = u != null ? u.EMPLOYEE_CODE : null
                           }).AsQueryable();

            return results.ToList<CustomerEntity>();
        }
        ...
        //                    CustomerType = c.TYPE,
        //                    CbsCardType = (from ct in _context.TB_M_CARD_TYPE
        //                                   where ct.CARD_TYPE_CODE == c.CARD_TYPE_CODE
        //                                   select new CbsCardTypeEntity
        //                                   {
        //                                       CardTypeID = ct.CARD_TYPE_ID,
        //                                       CardTypeCode = ct.CARD_TYPE_CODE,
            ...
            try
            {
                using (var transaction = _context.Database.BeginTransaction(IsolationLevel.ReadCommitted))
                {
                    ...
                    try
                    {
                        ...
                        if (customerEntity.CbsCardType != null && customerEntity.CbsCardType.CardTypeID != 0)
                        {
                            cardTypeCode = customerEntity.CbsCardType.CardTypeCode;
                        }
                        ...
                        if (customerEntity.CustomerId == null || customerEntity.CustomerId == 0)
                        {
                            ...
                            if (customerEntity.CbsCardType.CardTypeID > 0)
                                customer.CARD_TYPE_ID = customerEntity.CbsCardType.CardTypeID;
                            ...
                            string accountNoDummy = dbAccount.ACCOUNT_NO;
                            ...
                            if (customerEntity.CbsCardType.CardTypeID > 0)
                            {
                                contactEntity.CbsCardType = new CbsCardTypeEntity
                                {
                                    CardTypeID = customerEntity.CbsCardType.CardTypeID,
                                    CardTypeCode = customerEntity.CbsCardType.CardTypeCode
                                };
                            }
                            ...
                            contactEntity.CustomerNumber = customerEntity.CustomerNumber;
                            ...
                            lstCustomerContact.Add(new CustomerContactEntity
                            {
                                ...
                                Account = new AccountEntity { AccountId = accountDummy, AccountNo= accountNoDummy },
                        ...
                        {
                            //Edit
                            customer = _context.TB_M_CUSTOMER.FirstOrDefault(x => x.CUSTOMER_ID == customerEntity.CustomerId);
                            if (customer != null)
                            {
                                if (customerEntity.CbsCardType.CardTypeID > 0)
                                {
                                ...
                                // AddCustomerLog
                                this.AddCustomerLog(customer.CUSTOMER_ID, null, Constants.CustomerLog.EditCustomer,
        ...
        public bool IsDuplicateCardNo(int? customerId, int cbsCardTypeId, string cardNo)
        {
            string cardTypeCode = _context.TB_M_CARD_TYPE.FirstOrDefault(x => x.CARD_TYPE_ID == cbsCardTypeId).CARD_TYPE_CODE;
        ...
        public IEnumerable<NoteEntity> GetNoteList(NoteSearchFilter searchFilter)
        {
            ...
            if (searchFilter.CustomerId.HasValue && searchFilter.CustomerId.Value > 0)
                query = query.Where(a => a.CustomerId == searchFilter.CustomerId.Value);
            if (searchFilter.CustomerNumber.HasValue && searchFilter.CustomerNumber.Value > 0)
                query = query.Where(a => a.CustomerNumber == searchFilter.CustomerNumber.Value);
            if (searchFilter.EffectiveDate.HasValue == true)
                query = query.Where(a => a.EffectiveDate <= searchFilter.EffectiveDate.Value && a.ExpiryDate >= searchFilter.EffectiveDate.Value);
        ...
        public bool SaveNote(NoteEntity noteEntity)
        {
            ...
            try
            {
                ...
                if (noteEntity.NoteId == null || noteEntity.NoteId == 0)
                {
                    ...
                    note = new TB_M_NOTE();
                    note.KKCIS_ID = noteEntity.CustomerNumber;
        ...
        public IEnumerable<AttachmentEntity> GetAttachmentList(AttachmentSearchFilter searchFilter)
        {
            var query = from at in _context.TB_M_CUSTOMER_ATTACHMENT
                        ...
                        where (at.CUSTOMER_ID == searchFilter.CustomerId || at.KKCIS_ID == searchFilter.CustomerNumber)
            ...
            var querySR = from at in _context.TB_T_SR_ATTACHMENT
                          from usr in _context.TB_R_USER.Where(x => x.USER_ID == at.CREATE_USER).DefaultIfEmpty()
                          from cust in _context.TB_T_SR_CUSTOMER.Where(x => x.SR_ID == at.TB_T_SR.SR_ID).DefaultIfEmpty()
                          where at.TB_T_SR.TB_C_SR_STATUS.SR_STATUS_CODE != Constants.SRStatusCode.Draft
                          && (cust.CARD_TYPE_NAME == searchFilter.CbsCardTypeName && cust.CARD_NO == searchFilter.CardNo)
                          select new AttachmentEntity
                          {
        ...
        public void SaveCustomerAttachment(AttachmentEntity attachment)
        {
            ...
            var isIDNull = attachment.AttachmentId.Equals(null) || attachment.AttachmentId == 0 ? true : false;
            ...
            if (isIDNull)
            {
                ...
                dbAttach.KKCIS_ID = attachment.CustomerNumber;
                ...
                // AddCustomerLog
                this.AddCustomerLog(attachment.CustomerId, attachment.CustomerNumber, Constants.CustomerLog.AddDocument, attachment.CreateUserId.Value);
            ...
            if (!isIDNull)
            {
                ...
                // AddCustomerLog
                this.AddCustomerLog(dbAttach.CUSTOMER_ID, attachment.CustomerNumber, Constants.CustomerLog.EditDocument, attachment.CreateUserId.Value);
            }
        ...
        public void DeleteCustomerAttachment(int attachmentId, int updateBy)
        {
            try
            {
                ...
                if (custAttach != null)
                {
                    ...
                    // AddCustomerLog
                    this.AddCustomerLog(custAttach.CUSTOMER_ID, custAttach.KKCIS_ID, Constants.CustomerLog.DeleteDocument, updateBy);
                }
        ...
        private static IQueryable<AccountEntity> SetAccountListSort(IQueryable<AccountEntity> accountList, AccountSearchFilter searchFilter)
        {
            if (searchFilter.SortOrder.ToUpper(CultureInfo.InvariantCulture).Equals("ASC"))
            {
                switch (searchFilter.SortField)
                {
                    ...
                    case "CarNo":
                        return accountList.OrderBy(ord => ord.CarNo);
                    case "BranchName":
                    ...
                    case "Status":
                        return accountList.OrderBy(ord => (ord.AccountStatus == "A") ? "A" : "I");
                    default:
                        return accountList.OrderBy(ord => ord.ProductGroup);
                }
            }
            else
            {
                switch (searchFilter.SortField)
                {
                    ...
                    case "CarNo":
                        return accountList.OrderByDescending(ord => ord.CarNo);
                    case "BranchName":
                    ...
                    case "Status":
                        return accountList.OrderByDescending(ord => (ord.AccountStatus == "A") ? "A" : "I");
                    default:
                        return accountList.OrderByDescending(ord => ord.ProductGroup);
                }
            }
        ...
        //public IEnumerable<ServiceRequestCustomerSearchResult> GetCustomerAccountList(IEnumerable<CustomerEntity> custList, List<AccountEntity> prodList)
        //{
        //    var model = (from c in custList
        //                 from p in prodList.Where(x => x.CustomerNumber == c.CustomerNumber).DefaultIfEmpty()
        //                 from ac in _context.TB_M_ACCOUNT.Where(ac => (ac.CUSTOMER_ID > 0 && ac.CUSTOMER_ID == c.CustomerId)).DefaultIfEmpty()
        ...
        //                     AccountNo = p != null ? p.AccountNo : ac.ACCOUNT_NO,
        //                     CardNo = c.CardNo,
        ...
        //                     AccountStatus = p != null ? p.AccountStatus : ac.ac,
        ...
        {
            var query = from cc in _context.TB_M_CUSTOMER_CONTACT.AsNoTracking()
                        ...
                        from rl in _context.TB_M_RELATIONSHIP.Where(x => x.RELATIONSHIP_ID == cc.RELATIONSHIP_ID).DefaultIfEmpty()
                            //where (searchFilter.CustomerId != 0 && cc.CUSTOMER_ID == searchFilter.CustomerId)
                            //    || (searchFilter.CustomerNumber != 0 && cc.KKCIS_ID == searchFilter.CustomerNumber)
                        select new ContactEntity
                        {
                            CustomerId = cc.CUSTOMER_ID,
                            CustomerNumber = cc.KKCIS_ID,
                            ...
                            AccountNo = cc.ACCOUNT_NO,
                            AccountType = cc.ACCOUNT_TYPE,
                            Account = new AccountEntity { AccountId = cc.ACCOUNT_ID, AccountNo = cc.ACCOUNT_NO },
                            CbsCardType = (from ct in _context.TB_M_CARD_TYPE
                                           where ct.CARD_TYPE_CODE == con.CARD_TYPE_CODE
                                           select new CbsCardTypeEntity
                                           {
                                               CardTypeID = ct.CARD_TYPE_ID,
                                               CardTypeCode = ct.CARD_TYPE_CODE,
            ...
            if (searchFilter.CustomerId != 0)
                query = query.Where(a => a.CustomerId == searchFilter.CustomerId);
            if (searchFilter.CustomerNumber != 0)
                query = query.Where(a => a.CustomerNumber == searchFilter.CustomerNumber);
        ...
        private static IQueryable<ContactEntity> SetContactListSort(IQueryable<ContactEntity> accountList, ContactSearchFilter searchFilter)
        {
            if (searchFilter.SortOrder.ToUpper(CultureInfo.InvariantCulture).Equals("ASC"))
            {
                switch (searchFilter.SortField)
                {
                    ...
                    case "Status":
                        return accountList.OrderBy(ord => (ord.Account.AccountStatus == "A") ? "A" : "I");
                    default:
                        return accountList.OrderBy(ord => ord.CardNo);
                }
            }
            else
            {
                switch (searchFilter.SortField)
                {
                    ...
                    case "Status":
                        return accountList.OrderByDescending(ord => (ord.Account.AccountStatus == "A") ? "A" : "I");
                    default:
                        return accountList.OrderByDescending(ord => ord.CardNo);
                }
            }
        ...
        public bool DeleteCustomerContact(int customerContactId, int updateBy)
        {
            ...
            try
            {
                ...
                {
                    // AddCustomerLog
                    this.AddCustomerLog(objCustomerContact.CUSTOMER_ID, objCustomerContact.KKCIS_ID, Constants.CustomerLog.DeleteContact, updateBy);
        ...
        private bool SaveContact(ContactEntity contactEntity, List<CustomerContactEntity> lstCustomerContact, bool isAutoFromCustomer = false)
        {
            ...
            try
            {
                ...
                if (contactEntity.ContactId == null || contactEntity.ContactId == 0)
                {
                    ...
                    if(contactEntity.CbsCardType != null && contactEntity.CbsCardType.CardTypeID > 0) { 
                        contact.CARD_TYPE_ID = contactEntity.CbsCardType.CardTypeID;
                        contact.CARD_TYPE_CODE = contactEntity.CbsCardType.CardTypeCode;
                    }
                    ...
                    contact.KKCIS_ID = contactEntity.CustomerNumber;
                    ...
                    // AddCustomerLog
                    this.AddCustomerLog(contactEntity.CustomerId, contactEntity.CustomerNumber, Constants.CustomerLog.AddContact, contactEntity.UpdateUser.UserId);
                ...
                else
                {
                    ...
                    {
                        if (contactEntity.CbsCardType.CardTypeID > 0)
                        {
                            contact.CARD_TYPE_ID = contactEntity.CbsCardType.CardTypeID;
                            contact.CARD_TYPE_CODE = contactEntity.CbsCardType.CardTypeCode;
                        }
                        ...
                        // AddCustomerLog
                        this.AddCustomerLog(contactEntity.CustomerId.Value, contactEntity.CustomerNumber.Value, Constants.CustomerLog.EditContact, contactEntity.UpdateUser.UserId);
        ...
        public bool SaveContactSr(ContactEntity contactEntity)
        {
            ...
            try
            {
                ...
                if (!contactEntity.ContactId.HasValue)
                {
                    //Save New
                    contact = new TB_M_CONTACT();
                    if (contactEntity.CbsCardType.CardTypeID != 0)
                ...
                {
                    contact = _context.TB_M_CONTACT.Single(x => x.CONTACT_ID == contactEntity.ContactId.Value);
                }
                ...
                dbCustContact.KKCIS_ID = contactEntity.CustomerNumber;
                dbCustContact.ACCOUNT_NO = contactEntity.AccountNo;
                dbCustContact.ACCOUNT_TYPE = contactEntity.AccountType;
        ...
        private void SaveContactRelationship(List<CustomerContactEntity> lstCustomerContact, int contactId, int updateUser)
        {
            foreach (CustomerContactEntity customerContactEntity in lstCustomerContact)
            {
                ...
                if (isIDNull)
                {
                    ...
                    if (customerContactEntity.CustomerId.Value != 0)
                        dbCustContact.CUSTOMER_ID = customerContactEntity.CustomerId.Value;

                    if (customerContactEntity.Account.AccountId != null)
                        dbCustContact.ACCOUNT_ID = customerContactEntity.Account.AccountId;

                    dbCustContact.RELATIONSHIP_ID = customerContactEntity.RelationshipId;
                    dbCustContact.ACCOUNT_NO = customerContactEntity.Account.AccountNo;
                    dbCustContact.ACCOUNT_TYPE = customerContactEntity.Account.AccountType;

                    if (customerContactEntity.CustomerNumber != null  && customerContactEntity.CustomerNumber.Value != 0)
                        dbCustContact.KKCIS_ID = customerContactEntity.CustomerNumber;
                ...
                else
                {
                    ...
                    {
                        if (customerContactEntity.Account != null)
                        {
                            if (customerContactEntity.Account.AccountId != null)
                                dbCustContact.ACCOUNT_ID = customerContactEntity.Account.AccountId;
                            dbCustContact.ACCOUNT_NO = customerContactEntity.Account.AccountNo;
                        ...
                        {
                            dbCustContact.ACCOUNT_NO = customerContactEntity.AccountNo;
                            dbCustContact.ACCOUNT_TYPE = customerContactEntity.AccountType;
                        }
                        dbCustContact.RELATIONSHIP_ID = customerContactEntity.RelationshipId;
                        dbCustContact.UPDATE_USER = updateUser;
                        dbCustContact.UPDATE_DATE = DateTime.Now;
        ...
        public List<CustomerContactEntity> GetContactRelationshipList(int contactId, int currentCustomerId)
        {
            ...
            var query = from cc in _context.TB_M_CUSTOMER_CONTACT
                        ...
                        select new CustomerContactEntity
                        {
                            ...
                            AccountNo = cc.ACCOUNT_NO,
                            AccountType = cc.ACCOUNT_TYPE,
                            Account = new AccountEntity { AccountId = ac.ACCOUNT_ID, AccountNo = cc.ACCOUNT_NO, AccountDesc = ac.ACCOUNT_DESC },
                            Product = ac.SUBSCRIPTION_DESC,
        ...
        public IEnumerable<SrEntity> GetSrList(SrSearchFilter searchFilter)
        {
            ...
            var query = from sr in _context.TB_T_SR.AsNoTracking()
                        ...
                        from ac in _context.TB_T_SR_ACCOUNT.Where(x => x.SR_ID == sr.SR_ID).DefaultIfEmpty()
        ...
        public IEnumerable<SrEntity> GetCustomerSrList(SrSearchFilter searchFilter)
        {
            ...
            var query = from sr in _context.TB_T_SR
                        ...
                        from ownUsr in _context.TB_R_USER.Where(x => x.USER_ID == sr.OWNER_USER_ID).DefaultIfEmpty()
                        from dgUsr in _context.TB_R_USER.Where(x => x.USER_ID == sr.DELEGATE_USER_ID).DefaultIfEmpty()
                        from cust in _context.TB_T_SR_CUSTOMER.Where(x => x.SR_ID == sr.SR_ID).DefaultIfEmpty()
                        from state in _context.TB_C_SR_STATE.Where(x => x.SR_STATE_ID == sr.TB_C_SR_STATUS.SR_STATE_ID).DefaultIfEmpty()
                        from custAcct in _context.TB_T_SR_ACCOUNT.Where(x => x.SR_ID == sr.SR_ID).DefaultIfEmpty()
                            //where (cust.CARD_TYPE_NAME == searchFilter.CbsCardTypeName && cust.CARD_NO == searchFilter.CardNo)
                        where (cust.CARD_TYPE_NAME == searchFilter.CbsCardTypeName && ((searchFilter.CustomerId ?? 0) == 0 || cust.CUSTOMER_ID == searchFilter.CustomerId)
                                && ((searchFilter.CustomerNumber ?? 0) == 0 || cust.KKCIS_ID == searchFilter.CustomerNumber))
                        && (isNotFilterOwner == 1 || sr.SR_STATUS_ID == Constants.SRStatusId.Draft ||
                            (
                                (sr.OWNER_USER_ID.HasValue && lstFilerOwnerUserId.Contains(sr.OWNER_USER_ID.Value))
                                    || (sr.DELEGATE_USER_ID.HasValue && lstFilerOwnerUserId.Contains(sr.DELEGATE_USER_ID.Value)
        ...
        public IEnumerable<CustomerLogEntity> GetCustomerLogList(CustomerLogSearchFilter searchFilter)
        {
            ...
            if (searchFilter.CustomerId.HasValue && searchFilter.CustomerId > 0)
                query = query.Where(a => a.CustomerId == searchFilter.CustomerId);
            if (searchFilter.CustomerNumber.HasValue && searchFilter.CustomerNumber > 0)
                query = query.Where(a => a.CustomerNumber == searchFilter.CustomerNumber);
        ...
        private void AddCustomerLog(int? customerId, decimal? CustomerNumber, string detail, int createUser)
        {
            TB_L_CUSTOMER_LOG custLog = new TB_L_CUSTOMER_LOG();
            custLog.CUSTOMER_ID = customerId;
            custLog.KKCIS_ID = CustomerNumber;
        ...
        public IEnumerable<PhoneTypeEntity> GetPhoneType(int? id, string code, string name) => from p in _context.TB_M_PHONE_TYPE.AsNoTracking()
                                                                                               where (!id.HasValue || p.PHONE_TYPE_ID == id)
                                                                                               && ((code ?? "") == "" || p.PHONE_TYPE_CODE == code)
                                                                                               && ((name ?? "") == "" || p.PHONE_TYPE_NAME == name)
                                                                                               select new PhoneTypeEntity()
                                                                                               {
                                                                                                   PhoneTypeId = p.PHONE_TYPE_ID,
                                                                                                   PhoneTypeCode = p.PHONE_TYPE_CODE,
                                                                                                   PhoneTypeName = p.PHONE_TYPE_NAME
                                                                                               };
        ...
        private static IQueryable<CustomerEntity> SetCustomerListSort(IQueryable<CustomerEntity> customerList, CustomerSearchFilter searchFilter)
        {
            if (searchFilter.SortOrder.ToUpper(CultureInfo.InvariantCulture).Equals("ASC"))
            {
                switch (searchFilter.SortField)
                {
                    ...
                    case "Status":
                        return customerList.OrderBy(ord => (ord.Account.AccountStatus == "A") ? "A" : "I");
                    case "BranchName":
                    ...
                    case "CardTypeName":
                        return customerList.OrderBy(ord => ord.CbsCardType.CardTypeName);
                    default:
            ...
            else
            {
                switch (searchFilter.SortField)
                {
                    ...
                    case "Status":
                        return customerList.OrderByDescending(ord => (ord.Account.AccountStatus == "A") ? "A" : "I");
                    case "BranchName":
                    ...
                    case "CardTypeName":
                        return customerList.OrderByDescending(ord => ord.CbsCardType.CardTypeName);
                    default:
