    ...
    public class ServiceRequestDataAccess : IServiceRequestDataAccess
    {
        ...
        public IEnumerable<ServiceRequestEntity> SearchServiceRequest(ServiceRequestSearchFilter searchFilter)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.CustomerFirstName))
            {
                ...
                if (!string.IsNullOrEmpty(originalKeyword) && originalKeyword != "*" && originalKeyword != "**")
                {
                    if (originalKeyword.StartsWith("*") && originalKeyword.EndsWith("*"))
                    {
                        ...
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.FIRST_NAME_TH.Contains(keyword) || cs.FIRST_NAME_EN.Contains(keyword)));
                    }
                    else if (originalKeyword.StartsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(1, originalKeyword.Length - 1);
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.FIRST_NAME_TH.EndsWith(keyword) || cs.FIRST_NAME_EN.EndsWith(keyword)));
                    }
                    else if (originalKeyword.EndsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(0, originalKeyword.Length - 1);
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.FIRST_NAME_TH.StartsWith(keyword) || cs.FIRST_NAME_EN.StartsWith(keyword)));
                    }
                    else
                    {
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.FIRST_NAME_TH == originalKeyword || cs.FIRST_NAME_EN == originalKeyword));
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(searchFilter.CustomerLastName))
            {
                ...
                if (!string.IsNullOrEmpty(originalKeyword) && originalKeyword != "*" && originalKeyword != "**")
                {
                    ...
                    {
                        var keyword = originalKeyword.Substring(1, originalKeyword.Length - 2);
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.LAST_NAME_TH.Contains(keyword) || cs.LAST_NAME_EN.Contains(keyword)));
                    }
                    else if (originalKeyword.StartsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(1, originalKeyword.Length - 1);
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.LAST_NAME_TH.EndsWith(keyword) || cs.LAST_NAME_EN.EndsWith(keyword)));
                    }
                    else if (originalKeyword.EndsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(0, originalKeyword.Length - 1);
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.LAST_NAME_TH.StartsWith(keyword) || cs.LAST_NAME_EN.StartsWith(keyword)));
                    }
                    else
                    {
                        query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.LAST_NAME_TH == originalKeyword || cs.LAST_NAME_EN == originalKeyword));
                    }
                }
            }
            ...
            {
                query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.CARD_NO == searchFilter.CustomerCardNo));
            }
            ...
            {
                query = query.Where(q => q.TB_T_SR_ACCOUNT.Any(ac => ac.ACCOUNT_NO == searchFilter.AccountNo));
            }

            //Filter : PhoneNo
            if (!string.IsNullOrEmpty(searchFilter.PhoneNo))
            {
                query = query.Where(q => q.TB_T_SR_CUSTOMER.Any(cs => cs.TB_T_SR_CUSTOMER_PHONE.Any(p => p.PHONE_NO == searchFilter.PhoneNo)));
            }
            ...
            var results = from q in query
                          ...
                          select new ServiceRequestEntity
                          {
                              ...
                              CustomerCardNo = q.TB_T_SR_CUSTOMER.FirstOrDefault().CARD_NO,
        ...
        public ServiceRequestSaveSrResult SaveServiceRequest(ServiceRequestForSaveEntity entity, bool isSaveDraft)
        {
            ...
            try
            {
                ...
                SaveSRCustomer(entity);
                SaveSRAccount(entity);
                Save();
        ...
        private void SaveSRCustomer(ServiceRequestForSaveEntity entity)
        {
            var remove = _context.TB_T_SR_CUSTOMER.Where(x => x.SR_ID == entity.SrId);
            _context.TB_T_SR_CUSTOMER.RemoveRange(remove);

            var srCust = new TB_T_SR_CUSTOMER();

            srCust.SR_ID = entity.SrId;
            srCust.CARD_NO = entity.CustomerCardNo;
            srCust.TITLE_NAME_TH = entity.CustomerTitleTh;
            srCust.FIRST_NAME_TH = entity.CustomerFNameTh;
            srCust.LAST_NAME_TH = entity.CustomerLNameTh;
            srCust.TITLE_NAME_EN = entity.CustomerTitleEn;
            srCust.FIRST_NAME_EN = entity.CustomerFNameEn;
            srCust.LAST_NAME_EN = entity.CustomerLNameEn;
            srCust.EMAIL = entity.CustomerEmail;
            srCust.EMPLOYEE_ID = entity.CustomerEmployeeId > 0 ? entity.CustomerEmployeeId : null;
            srCust.BIRTH_DATE =  entity.CustomerBirthDate.ParseDateTime("dd/MM/yyyy");
            srCust.CARD_TYPE_NAME = entity.CustomerCardTypeName;
            srCust.CUSTOMER_TYPE = entity.CustomerType;
            srCust.KKCIS_ID = entity.CustomerNo;
            srCust.CARD_TYPE_CODE = entity.CustomerCardTypeCode;
            srCust.CUSTOMER_ID = entity.CustomerId;

            srCust.CREATE_USER = entity.CreateUserId;
            srCust.CREATE_DATE = DateTime.Now;

            _context.TB_T_SR_CUSTOMER.Add(srCust);
            Save();

            SaveSRCustomerPhone(srCust.SR_CUSTOMER_ID, entity);
            if (!string.IsNullOrWhiteSpace(entity.CustomerFax))
            {
                var srPhone = new TB_T_SR_CUSTOMER_PHONE();

                srPhone.SR_CUSTOMER_ID = srCust.SR_CUSTOMER_ID;
                srPhone.PHONE_NO = entity.CustomerFax;
                srPhone.PHONE_TYPE_ID = (_context.TB_M_PHONE_TYPE.Where(x => x.PHONE_TYPE_NAME == "เบอร์ FAX")
                                            ?.Select(x => x.PHONE_TYPE_ID)
                                            ?.FirstOrDefault()) ?? 5; //เบอร์ FAX

                srPhone.CREATE_USER = entity.CreateUserId;
                srPhone.CREATE_DATE = DateTime.Now;

                _context.TB_T_SR_CUSTOMER_PHONE.Add(srPhone);
                Save();
            }
        }

        private void SaveSRCustomerPhone(int srCustId, ServiceRequestForSaveEntity entity)
        {
            SaveSRCustomerPhone(srCustId, entity.CustomerPhoneTypeName1, entity.CustomerPhoneNo1, entity.CreateUserId);
            SaveSRCustomerPhone(srCustId, entity.CustomerPhoneTypeName2, entity.CustomerPhoneNo2, entity.CreateUserId);
            SaveSRCustomerPhone(srCustId, entity.CustomerPhoneTypeName3, entity.CustomerPhoneNo3, entity.CreateUserId);
        }

        private void SaveSRCustomerPhone(int srCustId, string phoneType, string phoneNo, int? uid)
        {
            if (!string.IsNullOrWhiteSpace(phoneNo))
            {
                var srPhone = new TB_T_SR_CUSTOMER_PHONE();

                srPhone.SR_CUSTOMER_ID = srCustId;
                srPhone.PHONE_NO = phoneNo;
                srPhone.PHONE_TYPE_ID = (_context.TB_M_PHONE_TYPE.Where(x => x.PHONE_TYPE_NAME == phoneType)
                                            ?.Select(x => x.PHONE_TYPE_ID)
                                            ?.FirstOrDefault()) ?? 2; //Default เบอร์มือถือ

                srPhone.CREATE_USER = uid;
                srPhone.CREATE_DATE = DateTime.Now;
        ...
        private void SaveSRAccount(ServiceRequestForSaveEntity entity)
        {
            var remove = _context.TB_T_SR_ACCOUNT.Where(x => x.SR_ID == entity.SrId);
            _context.TB_T_SR_ACCOUNT.RemoveRange(remove);

            var srAcc = new TB_T_SR_ACCOUNT();

            srAcc.SR_ID = entity.SrId.Value;
            srAcc.KKCIS_ID = entity.CustomerNo;
            srAcc.ACCOUNT_NO = entity.AccountNo;
            srAcc.PRODUCT_GROUP = entity.AccountProductGroup;
            srAcc.PRODUCT = entity.AccountProduct;
            srAcc.STATUS = entity.AccountStatusId.ToString();
            srAcc.BRANCH_NAME = entity.AccountBranchName;
            srAcc.BRANCH_CODE = entity.AccountBranchCode;

            srAcc.UPDATE_DATE = DateTime.Now;
            _context.TB_T_SR_ACCOUNT.Add(srAcc);
        }
        ...
        //        CustomerType = q.TB_M_CUSTOMER.TYPE ?? 0,
        //        CustomerCategory = q.TB_M_CUSTOMER.tb_cu,
        //        SubscriptionTypeName = q.TB_M_CUSTOMER.TB_M_SUBSCRIPT_TYPE.SUBSCRIPT_TYPE_NAME,
        //        SubscriptType = new SubscriptTypeEntity
        //        {
        //            SubscriptTypeId = q.TB_M_CUSTOMER.SUBSCRIPT_TYPE_ID,
        //            SubscriptTypeCode = q.TB_M_CUSTOMER.TB_M_SUBSCRIPT_TYPE.SUBSCRIPT_TYPE_CODE,
        //            SubscriptTypeName = q.TB_M_CUSTOMER.TB_M_SUBSCRIPT_TYPE.SUBSCRIPT_TYPE_NAME
        //        },
        ...
        public ServiceRequestCustomerAccount GetCustomerAccount(int? accountId, int? customerId = null)
        {
            var query = from a in _context.TB_M_ACCOUNT
                        ...
                        from ct in _context.TB_M_CARD_TYPE.Where(x => x.CARD_TYPE_ID == c.CARD_TYPE_ID).DefaultIfEmpty()
                        ...
                        where a.ACCOUNT_ID == accountId && (!customerId.HasValue || a.CUSTOMER_ID == customerId.Value)
                        select new ServiceRequestCustomerAccount
                        {
                            ...
                            Customer = new CustomerEntity()
                            {
                                ...
                                CbsCardType = (ct != null ? new CbsCardTypeEntity
                                {
        ...
        public IEnumerable<ServiceRequestAccountSearchResult> SearchAccount(ContractSearchFilter searchFilter)
        {
            ...
            if ((searchFilter.CustomerNo ?? 0) > 0)
                query = query.Where(q => q.KKCIS_ID == searchFilter.CustomerNo.Value);
            else
                query = query.Where(q => q.CUSTOMER_ID == searchFilter.CustomerId);
        ...
        {
            var query = _context.TB_M_CUSTOMER_CONTACT.AsNoTracking().AsQueryable();
            if ((searchFilter.CustomerNo ?? 0) > 0)
                query = query.Where(q => q.KKCIS_ID == searchFilter.CustomerNo && q.RELATIONSHIP_ID != null);
            else
            ...
            var resultQuery = query.Select(q => new ServiceRequestContactSearchResult
            {
                ...
                AccountNo = (q.ACCOUNT_ID ?? 0) > 0 ? _context.TB_M_ACCOUNT.Where(x => x.ACCOUNT_ID == q.ACCOUNT_ID).FirstOrDefault().ACCOUNT_NO : q.ACCOUNT_NO,
                Product = (q.ACCOUNT_ID ?? 0) > 0 ? _context.TB_M_ACCOUNT.Where(x => x.ACCOUNT_ID == q.ACCOUNT_ID).FirstOrDefault().SUBSCRIPTION_DESC : string.Empty,
                ...
                CbsCardTypeName = q.TB_M_CONTACT.TB_M_CARD_TYPE.CARD_TYPE_NAME,
        ...
        public ServiceRequestForDisplayEntity GetServiceRequest(int srId)
        {
            ...
            var result = new ServiceRequestForDisplayEntity
            {
                ...
                Customer = new CustomerEntity
                {
                    ...
                    PhoneList = _context.TB_T_SR_CUSTOMER_PHONE.Where(x => x.SR_CUSTOMER_ID == sr.SR_CUSTOMER_ID && x.PHONE_TYPE_ID != 5).Select(x => new PhoneEntity()
                    {
                        PhoneNo = x.PHONE_NO,
                        PhoneTypeId = x.PHONE_TYPE_ID,
                        PhoneTypeName = x.TB_M_PHONE_TYPE.PHONE_TYPE_NAME
                    }).ToList(),
                    Fax = _context.TB_T_SR_CUSTOMER_PHONE.Where(x => x.SR_CUSTOMER_ID == sr.SR_CUSTOMER_ID && x.PHONE_TYPE_ID == 5).Select(x => x.PHONE_NO).FirstOrDefault()
                },
                CustomerCardType = new CbsCardTypeEntity
                {
                    CardTypeID = Convert.ToInt32( sr.CUSTOMER_SUBSCRIPT_TYPE_ID),
                    CardTypeCode = sr.CUSTOMER_SUBSCRIPT_TYPE_CODE,
                    CardTypeName = sr.CUSTOMER_SUBSCRIPT_TYPE_NAME
                },
                ...
                Account = new AccountEntity
                {
                    ...
                    AccountStatus = sr.ACCOUNT_STATUS_ID,
                    AccountStatusDescription = sr.ACCOUNT_STATUS_NAME,
                ...
                ContactCardType = sr.CONTACT_CARD_TYPE_ID.HasValue ? new CbsCardTypeEntity
                {
                    CardTypeID = Convert.ToInt32(sr.CONTACT_CARD_TYPE_ID),
                    CardTypeCode = sr.CONTACT_CARD_TYPE_CODE,
                    CardTypeName = sr.CONTACT_CARD_TYPE_NAME
                } : null,
        ...
        public ServiceRequestForDisplayEntity GetServiceRequestOld(int srId)
        {
            return (from sr in _context.TB_T_SR
                    ...
                    from customerCardType in _context.TB_M_CARD_TYPE.Where(x => x.CARD_TYPE_ID == customer.CARD_TYPE_ID).DefaultIfEmpty()
                    ...
                    from contactCardType in _context.TB_M_CARD_TYPE.Where(x => x.CARD_TYPE_ID == contact.CARD_TYPE_ID).DefaultIfEmpty()
                    ...
                    select new ServiceRequestForDisplayEntity
                    {
                        ...
                        CustomerCardType = customerCardType != null ? new CbsCardTypeEntity
                        {
                            CardTypeID = customerCardType.CARD_TYPE_ID,
                            CardTypeCode = customerCardType.CARD_TYPE_CODE,
                            CardTypeName = customerCardType.CARD_TYPE_NAME
                        } : null,
                        ...
                        ContactCardType = contactCardType != null ? new CbsCardTypeEntity
                        {
                            CardTypeID = contactCardType.CARD_TYPE_ID,
                            CardTypeCode = contactCardType.CARD_TYPE_CODE,
                            CardTypeName = contactCardType.CARD_TYPE_NAME,
                        } : null,
        ...
        public CustomerContactEntity GetCustomerContact(int id)
        {
            return (from cc in _context.TB_M_CUSTOMER_CONTACT
                    ...
                    select new CustomerContactEntity
                    {
                        ...
                        Account = new AccountEntity { AccountId = ac.ACCOUNT_ID, AccountNo = cc.ACCOUNT_NO },
                        ...
                        CustomerId = cust != null ? cust.CUSTOMER_ID : (int?)null,
        ...
        public CustomerContactEntity GetCustomerContactByContactId(int contactId)
        {
            return (from cc in _context.TB_M_CUSTOMER_CONTACT
                    from con in _context.TB_M_CONTACT.Where(x => x.CONTACT_ID == cc.CONTACT_ID).DefaultIfEmpty()
                    from ac in _context.TB_M_ACCOUNT.Where(x => x.ACCOUNT_ID == cc.ACCOUNT_ID).DefaultIfEmpty()
        ...
        public IQueryable<ServiceRequestActivityResult> GetSrActivityQueryable(bool isSrOnly, int? srId, int? customerId, decimal? cifNo)
        {
            return (from sa in _context.TB_T_SR_ACTIVITY
                    from sr in _context.TB_T_SR.Where(x => x.SR_ID == sa.SR_ID).DefaultIfEmpty()
                        //from customer in _context.TB_M_CUSTOMER.Where(x => x.CUSTOMER_ID == sr.CUSTOMER_ID).DefaultIfEmpty()
                        //from account in _context.TB_M_ACCOUNT.Where(x => x.ACCOUNT_ID == sr.ACCOUNT_ID).DefaultIfEmpty()
                    from customer in _context.TB_T_SR_CUSTOMER.Where(x => x.SR_ID == sr.SR_ID).DefaultIfEmpty()
                    from account in _context.TB_T_SR_ACCOUNT.Where(x => x.SR_ID == sr.SR_ID).DefaultIfEmpty()
                    ...
                    where (isSrOnly ? sa.SR_ID == srId.Value : (((customerId ?? 0) == 0 || customer.CUSTOMER_ID == customerId.Value)
                                                                && ((cifNo ?? 0) == 0 || customer.KKCIS_ID == cifNo.Value)) )
        ...
        {
            var query = GetSrActivityQueryable(searchFilter.SrOnly, searchFilter.SrId, searchFilter.CustomerId, searchFilter.CustomerNo);
            searchFilter.TotalRecords = query.Count();
        ...
        public IEnumerable<ServiceRequestEntity> GetTabExistingList(ExistingSearchFilter searchFilter)
        {
            ...
            var results = from q in _context.TB_T_SR.AsNoTracking()
                          from ownerUser in _context.TB_R_USER.Where(user => q.OWNER_USER_ID == user.USER_ID).DefaultIfEmpty()
                          from delegateUser in _context.TB_R_USER.Where(user => q.DELEGATE_USER_ID == user.USER_ID).DefaultIfEmpty()
                          from srCust in _context.TB_T_SR_CUSTOMER.Where(x => q.SR_ID == x.SR_ID).DefaultIfEmpty()
                          from srAcct in _context.TB_T_SR_ACCOUNT.Where(x => q.SR_ID == x.SR_ID).DefaultIfEmpty()
                          where ((searchFilter.CustomerId ?? 0) == 0 || srCust.CUSTOMER_ID == searchFilter.CustomerId)
                            && ((searchFilter.CustomerNo ?? 0) == 0 || srCust.KKCIS_ID == searchFilter.CustomerNo)
                          select new ServiceRequestEntity
                          {
                              SrId = q.SR_ID,
                              SrNo = q.SR_NO,
                              ...
                              NextSLA = q.RULE_NEXT_SLA,
                              TotalWorkingHours = q.RULE_TOTAL_WORK,
                              CustomerCardNo = srCust.CARD_NO, //q.TB_M_CUSTOMER.CARD_NO,
                              AccountNo = srAcct.ACCOUNT_NO, //q.TB_M_ACCOUNT != null ? q.TB_M_ACCOUNT.ACCOUNT_NO : string.Empty,
                              ChannelId = q.CHANNEL_ID,
                              ChannelName = q.TB_R_CHANNEL != null ? q.TB_R_CHANNEL.CHANNEL_NAME : string.Empty,
                              ProductId = q.PRODUCT_ID,
                              ProductName = srAcct.PRODUCT, //q.TB_R_PRODUCT != null ? q.TB_R_PRODUCT.PRODUCT_NAME : srAcct.PRODUCT,
                              CampaignServiceId = q.CAMPAIGNSERVICE_ID,
        ...
        public void CreateSrAttachment(List<SrAttachmentEntity> attachments, int srId, int srActivityId, int? customerId, int? createUserId, string createUsername)
        {
            ...
            foreach (var srAttachment in attachments)
            {
                ...
                if (srAttachment.IsEditable)
                {
                    ...
                    attach.CUSTOMER_ID = customerId == 0 ? null : customerId;
            ...
            if (srAttachmentNoCustomers.Count > 0)
            {
                ...
                {
                    item.CUSTOMER_ID = customerId == 0 ? null : customerId;
                }
                Save();
            }
        }
        ...
        public List<ServiceRequestCustomerSearchResult> GetCustomerAccountByPhoneNo(List<string> lstPhoneNo)
        {
            return _context.TB_M_ACCOUNT
                ...
                .Select(q => new ServiceRequestCustomerSearchResult
                {
                    ...
                    BranchName = q.BRANCH_NAME,
                    CustomerType = q.TB_M_CUSTOMER.TYPE ?? 0,
                    CardTypeName = q.TB_M_CUSTOMER.TB_M_CARD_TYPE != null ? q.TB_M_CUSTOMER.TB_M_CARD_TYPE.CARD_TYPE_NAME : string.Empty
                }).ToList();
        }
        ...
        public List<ServiceRequestCustomerSearchResult> GetCustomerByName(string customerTHName)
        {
            var result = from c in _context.TB_M_CUSTOMER
                         ...
                         select new ServiceRequestCustomerSearchResult
                         {
                             ...
                             CardTypeName = c.TB_M_CARD_TYPE.CARD_TYPE_NAME
                         };
        ...
        public ServiceRequestContactSearchResult GetCustomerContactById(int contactId)
        {
            ...
            var resultQuery = new ServiceRequestContactSearchResult
            {
                ...
                CbsCardTypeName = query.TB_M_CONTACT.TB_M_CARD_TYPE != null ? query.TB_M_CONTACT.TB_M_CARD_TYPE.CARD_TYPE_NAME : string.Empty,
                BirthDay = query.TB_M_CONTACT.BIRTH_DATE,
                TitleTh = query.TB_M_CONTACT.TB_M_TITLE != null ? query.TB_M_CONTACT.TB_M_TITLE.TITLE_NAME : string.Empty,
                TitleEn = query.TB_M_CONTACT.TB_M_TITLE != null ? query.TB_M_CONTACT.TB_M_TITLE1.TITLE_NAME : string.Empty, 
        ...
        public IEnumerable<ServiceRequestActivityResult> GetSRActivityList(ActivitySearchFilter searchFilter)
        {
            ...
            var query = GetSrActivityQueryable(false, 0, searchFilter.CustomerId, searchFilter.CustomerNo);
        ...
        public ServiceRequestForInsertLogEntity GetServiceRequestActivityForInsertLog(int srActivityId)
        {
            ...
            var activity = new ServiceRequestForInsertLogEntity
            {
                ...
                CustomerCardTypeId = result.CUSTOMER_CARD_TYPE_ID ?? 0,
                CustomerCardTypeName = result.CUSTOMER_CARD_TYPE_NAME,
                CustomerCardTypeCode = result.CUSTOMER_CARD_TYPE_CODE,
                ...
                ContactCardTypeName = result.CONTACT_CARD_TYPE_NAME,
        ...
        public List<SearchSRResponseItem> SearchServiceRequestWebService(SearchSRRequest searchFilter, ref int totalRecords)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.CustomerCardNo) && !string.IsNullOrEmpty(searchFilter.CustomerSubscriptionTypeCode))
            {
                query = query.Where(q =>
                    ...
                    && q.TB_M_CUSTOMER.TB_M_CARD_TYPE.CARD_TYPE_CODE == searchFilter.CustomerSubscriptionTypeCode);
            }
            ...
            if (!string.IsNullOrEmpty(searchFilter.ContactCardNo) && !string.IsNullOrEmpty(searchFilter.ContactSubscriptionTypeCode))
            {
                query = query.Where(q =>
                    ...
                    && q.TB_M_CONTACT.TB_M_CARD_TYPE.CARD_TYPE_CODE == searchFilter.ContactSubscriptionTypeCode);
            }
            ...
            var results = from q in query
                          ...
                          select new SearchSRResponseItem
                          {
                              ...
                              CustomerFirstNameEn = q.TB_M_CUSTOMER.FIRST_NAME_EN,
                              CustomerLastNameEn = q.TB_M_CUSTOMER.LAST_NAME_EN,
                              CustomerSubscriptionTypeCode = q.TB_M_CUSTOMER.TB_M_CARD_TYPE.CARD_TYPE_CODE,
                              CustomerSubscriptionTypeName = q.TB_M_CUSTOMER.TB_M_CARD_TYPE.CARD_TYPE_NAME,
                              CustomerCardNo = q.TB_M_CUSTOMER.CARD_NO,
                              AccountNo = q.TB_M_ACCOUNT.ACCOUNT_NO,
                              ContactSubscriptionTypeCode = q.TB_M_CONTACT.TB_M_CARD_TYPE.CARD_TYPE_CODE,
        ...
        public GetSRResponse GetServiceRequestWebService(string srNo)
        {
            ...
            var results = from q in query
                          ...
                          select new GetSRResponse()
                          {
                              ...
                              CustomerCardNo = q.TB_M_CUSTOMER.CARD_NO,
                              AccountNO = q.TB_M_ACCOUNT.ACCOUNT_NO,
                              ContactSubscriptionType = q.TB_M_CONTACT.TB_M_CARD_TYPE.CARD_TYPE_CODE,
                              ContactCardNo = q.TB_M_CONTACT.CARD_NO,
        ...
        public int? GetCustomerIdByCode(string subscriptionTypeCode, string customerCardNo)
        {
            ...
            var item = _context.TB_M_CUSTOMER
                .Where(x => x.TB_M_CARD_TYPE.CARD_TYPE_CODE.ToUpper() == subscriptionTypeCode
