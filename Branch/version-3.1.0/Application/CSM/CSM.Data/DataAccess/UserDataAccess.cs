        ...
        public InsertOrUpdateUserResponse InsertOrUpdateUser(InsertOrUpdateUserRequest request, int? supervisorUserId, int roleId, int branchId)
        {
            ...
            try
            {
                ...
                if (!request.IsGroup)
                {
                    ...
                    //Default Card Type Create and Update case
                    customerEntity.CbsCardType = (new CommonDataAccess(_context)).GetCbsCardTypeByCode(Constants.DefaultCardTypeForUser);
                    ...
                    if (isCreateCustomer)
                    {
                        ...
                        if (!string.IsNullOrEmpty(request.Phone1))
                            customerEntity.PhoneList.Add(new PhoneEntity() { PhoneNo = request.Phone1, PhoneTypeId = 1 }); //Default เบอร์โทรอื่นๆ

                        if (!string.IsNullOrEmpty(request.Phone2))
                            customerEntity.PhoneList.Add(new PhoneEntity() { PhoneNo = request.Phone2, PhoneTypeId = 1 });

                        if (!string.IsNullOrEmpty(request.Phone3))
                            customerEntity.PhoneList.Add(new PhoneEntity() { PhoneNo = request.Phone3, PhoneTypeId = 1 });
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < phones.Count; i++)
                        {
                            ...
                            customerEntity.PhoneList[i].PhoneTypeId = 1; //Default เบอร์โทรอื่นๆ
                        }
