...
namespace CSM.Business
{
    public class CustomerFacade : BaseFacade, ICustomerFacade
    {
        ...
        {
            _customerDataAccess = new CustomerDataAccess(_context);
            return _customerDataAccess.IsDuplicateCardNo(customerId, cbsCardTypeId, cardNo);
        }
        ...
        public List<PhoneTypeEntity> GetPhoneType(int? id = null, string code = null, string name = null)
        {
            return (new CustomerDataAccess(_context)).GetPhoneType(id, code, name).ToList();
        }
        ...
        public CustomerContactEntity GetCustomerContact(string customerType, string customerId, string customerNumber)
        {
            CustomerContactEntity ret = new CustomerContactEntity();
            if (string.IsNullOrWhiteSpace(customerType))
            {
                int custId;
                decimal custNo;
                if (int.TryParse(customerId, out custId) && custId > 0)
                {
                    customerType = GetCustomerByID(custId)?.CustomerType.ToString();
                }
                else if (decimal.TryParse(customerNumber, out custNo) && custNo > 0)
                {
                    customerType = Constants.CustomerType.Customer.ToString();
                }
            }
            string[] ccType = { "P", "F", "M" };
            if (customerType == Constants.CustomerType.Prospect.ToString() || customerType == Constants.CustomerType.Employee.ToString())
            {
                if (customerId != "")
                {
                    int custId = Convert.ToInt32(customerId);
                    ret.PhoneList = (from p in _context.TB_M_PHONE
                                     where ccType.Contains(p.TB_M_PHONE_TYPE.CBS_CONTACT_TYPE) 
                                     && p.TB_M_PHONE_TYPE.PHONE_TYPE_CODE != Constants.PhoneTypeCode.Fax
                    ...
                    ret.Fax = (from e in _context.TB_M_PHONE.Where(a => a.CUSTOMER_ID == custId && a.TB_M_PHONE_TYPE.PHONE_TYPE_CODE == Constants.PhoneTypeCode.Fax).AsQueryable() select e).FirstOrDefault()?.PHONE_NO;
                    ret.Email = (from e in _context.TB_M_CUSTOMER.Where(a => a.CUSTOMER_ID == custId).AsQueryable() select e).FirstOrDefault()?.EMAIL;
                }
            }
            else if(customerType == Constants.CustomerType.Customer.ToString() ){
                if (customerNumber != "")
                {
                    _commonFacade = new CommonFacade();
                    ...
                    var query = WSInquiryCustomerContact(Convert.ToDecimal(customerNumber), vPaging);
                    if (query != null)
                    {
                        var tmpPhone = (from c in query
                                        from pt in _context.TB_M_PHONE_TYPE.Where(a => a.CBS_ELECTRONIC_ADDRESS_CODE == c.ContactCode).DefaultIfEmpty()
                                        where (pt != null && ccType.Contains(pt.CBS_CONTACT_TYPE))
                                        && pt.PHONE_TYPE_CODE != Constants.PhoneTypeCode.Fax
                                        orderby pt.PHONE_TYPE_CODE ascending
                                        select new PhoneEntity
                                        {
                                            //CustomerId = p.CUSTOMER_ID,
                                            PhoneTypeId = pt != null ? pt.PHONE_TYPE_ID : (int?)null,
                                            //PhoneId = pt.PHONE_ID,
                                            PhoneNo = c.ContactDetail.Replace("+66", "0"),
                                            PhoneTypeName = pt != null ? pt.PHONE_TYPE_NAME : null
                                        });
                        ret.PhoneList = (tmpPhone.Any() ? tmpPhone.ToList() : new List<PhoneEntity>());

                        var tmpMail = (from c in query
                                       from pt in _context.TB_M_PHONE_TYPE.Where(a => a.CBS_ELECTRONIC_ADDRESS_CODE == c.ContactCode).DefaultIfEmpty()
                                       where (pt != null && pt.PHONE_TYPE_CODE == Constants.PhoneTypeCode.Email)
                                       select c);
                        ret.Email = (tmpMail.Any() ? tmpMail.FirstOrDefault().ContactDetail : "");

                        var tmpFax = (from c in query
                                      from pt in _context.TB_M_PHONE_TYPE.Where(a => a.CBS_ELECTRONIC_ADDRESS_CODE == c.ContactCode).DefaultIfEmpty()
                                      where (pt != null && pt.PHONE_TYPE_CODE == Constants.PhoneTypeCode.Fax)
                                      select c);
                        ret.Fax = (tmpFax.Any() ? tmpFax.FirstOrDefault().ContactDetail : "");
                    }
                    else
                    {
        ...
        #region Call web service static function

        public static InquiryCustomerContactResponseHeaderContact[] WSInquiryCustomerContact(decimal? customerNo, WebServicePagingEntity vPaging)
        {
            CBSCustomerService client = new CBSCustomerService();
            InquiryCustomerContact req = new InquiryCustomerContact();
            LogMessageBuilder _logMsg = new LogMessageBuilder();
            InquiryCustomerContactResponse res = new InquiryCustomerContactResponse();
            Logger.Info(_logMsg.Clear().SetPrefixMsg("WSInquiryCustomerContact").Add("customerNo:", customerNo));
            try
            {
                Header profile = null;
                ...
                {
                    profile = commonFacade.GetHeaderByServiceName<Header>(Constants.ServiceName.InquiryCustomerContact);
                }

                req.Header = new InquiryCustomerContactHeader()
                {
                ...
                req.Paging = new InquiryCustomerContactPaging()
                {
                    PageNumber = vPaging.PageNumber,
                    PageSize = vPaging.PageSize
                };

                req.InquiryCustomerContactRequest = new InquiryCustomerContactInquiryCustomerContactRequest();
                req.InquiryCustomerContactRequest.CustomerNumber = customerNo?.ToString();
                req.InquiryCustomerContactRequest.CategoryCode = "";
                req.InquiryCustomerContactRequest.InquiryMode = "";
                SaveServiceRequestLog(req.SerializeObject());

                res = client.InquiryCustomerContact(req);
                if (res != null && res.Header != null && res.Header.ResponseStatusInfo != null && res.Header.ResponseStatusInfo.ResponseStatus != null
                    && (res.Header.ResponseStatusInfo.ResponseStatus.ResponseCode == "CBS-I-1000" || res.Header.ResponseStatusInfo.ResponseStatus.ResponseCode == "CBS-M-2001"))
                {
                    Logger.Info(_logMsg.Clear().SetPrefixMsg("WSInquiryCustomerContact").Add("customerNo:", customerNo).ToSuccessLogString());
                    Logger.Info(_logMsg.Clear().SetPrefixMsg("WSInquiryCustomerContact").Add("customerNo:", customerNo).SetSuffixMsg($"Return {res.Header.ContactList.Count()} record(s)."));
                    
                    return res.Header.ContactList;
                }
                else
                {
                    if (res == null || res.Header == null || res.Header.ResponseStatusInfo == null && res.Header.ResponseStatusInfo.ResponseStatus == null)
                        Logger.Info(_logMsg.Clear().SetPrefixMsg("WSInquiryCustomerContact").Add("Error Message", "Services not return the response.").ToFailLogString());
                    else
                        Logger.Info(_logMsg.Clear().SetPrefixMsg("WSInquiryCustomerContact").Add("ResponseCode", res.Header.ResponseStatusInfo.ResponseStatus.ResponseCode)
                            .Add("ResponseMessage", res.Header.ResponseStatusInfo.ResponseStatus.ResponseMessage).ToFailLogString());
                }
            }
            catch (Exception ex)
            {
                Logger.Error("Exception occur:\n", ex);
                Logger.Info(_logMsg.Clear().SetPrefixMsg("WSInquiryCustomerContact").Add("Error Message", ex.Message).ToFailLogString());
            }
            finally
            {
        ...
        private static void SaveServiceRequestLog(string req)
        {
            CommonFacade _commonFacade = new CommonFacade();
            if (_commonFacade.GetFlagCbsSaveLog() == true)
            {
                Logger.DebugFormat("-- XMLRequest --\n{0}", req);
            }
        }
        private static void SaveServiceResponseLog(string res)
        {
            CommonFacade _commonFacade = new CommonFacade();
            if (_commonFacade.GetFlagCbsSaveLog() == true)
            {
