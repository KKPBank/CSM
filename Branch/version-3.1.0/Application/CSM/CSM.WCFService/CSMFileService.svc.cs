...
namespace CSM.WCFService
{
    ...
    public class CSMFileService : ICSMFileService
    {
        ...
        public ImportCBSMasterTaskResponse GetFileCBSMaster(string username, string password)
        {
            ImportCBSMasterTaskResponse taskResponse = null;
            try
            {
                ThreadContext.Properties["EventClass"] = ApplicationHelpers.GetCurrentMethod(1);
                ThreadContext.Properties["RemoteAddress"] = ApplicationHelpers.GetClientIP();
                ...
                string fiCbsActivity = string.Empty;
                DateTime schedDateTime = DateTime.Now;
                _stopwatch = System.Diagnostics.Stopwatch.StartNew();
                _logger.Debug("-- Start Cron Job --:--Get GetFileHP--");
                ...
                if (AppLog.BatchProcessStart(Constants.BatchProcessCode.ImportCBS, schedDateTime) == false)
                {
                    _logger.Info("I:--NOT PROCESS--:--GetFileCBS Master--");

                    _stopwatch.Stop();
                    _elapsedTime = _stopwatch.ElapsedMilliseconds;
                    _logger.Debug("-- Finish Cron Job --:ElapsedMilliseconds/" + _elapsedTime);

                    taskResponse = new ImportCBSMasterTaskResponse
                    {
                        SchedDateTime = schedDateTime,
                        ElapsedTime = _elapsedTime,
                        StatusResponse = new StatusResponse
                ...
                try {
                    _logger.Info("I:--START--:--Get GetFileCBS--");
                    ...
                    taskResponse = new ImportCBSMasterTaskResponse
                    {
                        ...
                        FileList = new List<object> { fiCbsActivity },
                        NumOfTotal = 0,
                        NumOfComplete = 0,
                        NumOfError = 0
                    };
            ...
            finally
            {
                    ...
                    taskResponse.StatusResponse.Status != Constants.StatusResponse.NotProcess)
                {
                    ...
                    AppLog.BatchProcessEnd(Constants.BatchProcessCode.ImportCBS, batchStatus, endTime, processTime, processDetail);
                }
        ...
        //    BdwFacade bdw = new BdwFacade();
        //    return bdw.TestFunction(numOfBdwContact, numOfError, messageError);
        //}
