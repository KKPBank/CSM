...
function showServerErrors2(result) {
    ...
    if (result.Error != '') {
        ...
        $jq.each(result.Errors, function (key, val) {
            ...
            if (inputCtrl.length !== 1) {
                var inputCtrl2 = $jq('[name$=".' + controlId + '"]');
                if (inputCtrl2.length !== 1) {
                    var inputCtrl3 = $jq('[name=' + controlId + ']');
...
function validateDateFormat(date) {
    ...
    var days = parseInt(strArray[0]);
    var months = parseInt(strArray[1]);
    var years = parseInt(strArray[2]);

    if (isNaN(days) || isNaN(months) || isNaN(years)) {
        no_error = false;
    }
    else if (months > 12 || months < 1 || days < 1) {
        no_error = false;
    }
    //ตรวจสอบเดือนที่มี 31 วัน
    ...
    //ตรวจสอบ leap year สำหรับเดือน กพ.
    else if (new Date(years, 1, 29).getMonth() === 1) {
        no_error = (days <= 29);
    }
    else {
        no_error = (days <= 28);
