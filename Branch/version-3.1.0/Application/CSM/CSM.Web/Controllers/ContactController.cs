...
namespace CSM.Web.Controllers
{
    ...
    public class ContactController : BaseController
    {
        ...
        public ActionResult InitEdit(int? relationshipId = null)
        {
            ...
            try
            {
                ...
                if (contactVM.RelationshipId != null)
                {
                    ...
                    contactVM.CreatedDate = relationshipEntity.CreatedDate.FormatDateTime(Constants.DateTimeFormat.DefaultFullDateTime);
                    contactVM.UpdateDate = relationshipEntity.Updatedate.FormatDateTime(Constants.DateTimeFormat.DefaultFullDateTime);
                    contactVM.CbsRelationShipCode = relationshipEntity.CbsRelationshipCode;
                }
                else
                {
                    // default UserLogin
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    RelationshipEntity relationEntity = new RelationshipEntity
                    {
                        ...
                        UpdateUser = UserInfo,
                        CbsRelationshipCode = contactVM.CbsRelationShipCode,
                    };
        ...
        public ActionResult List(string encryptedString)
        {
            ...
            Logger.Info(_logMsg.Clear().SetPrefixMsg("List Contact").Add("CustomerId", customerId).Add("CustomerNumber", customerNumber).ToInputLogString());
            ...
            try
            {
                ...
                {
                    contactVM.CustomerInfo = (CustomerInfoViewModel)TempData["CustomerInfo" + customerId.ToString() + customerNumber.ToString()];
                    TempData["CustomerInfo" + customerId.ToString() + customerNumber.ToString()] = contactVM.CustomerInfo;
                }
        ...
        AccountEntity MappingCASAAccountEntity(string AccountNumber, string CustomerNumber)
        {
            ProductServiceController pd = new ProductServiceController();
            AccountEntity result = null;
            CBSCasaAccountDetailResponse casa = pd.InquiryCASAAccountDetailResponseWS(AccountNumber, CustomerNumber);
            if (casa.ResponseCode == "CBS-I-1000" || casa.ResponseCode == "CBS-M-2001")
            {
                ...
                if (res.Header.AccountInfo != null)
                {
                    ...
                    decimal custNo;
                    AccountEntity pdList = null;
                    if (decimal.TryParse(CustomerNumber, out custNo) && custNo > 0)
                    {
                        pdList = pd.ProductServiceListWS(new AccountSearchFilter() { CustomerNumber = custNo })?
                            .Where(x => x.AccountNo == AccountNumber).FirstOrDefault();
                    }
                    result = new AccountEntity
                    {
                        ProductGroup = acc.LocalDescription,
                        AccountDesc = acc.AccountName,
                        AccountNo = acc.AccountNumber,
                        //Grade = acc.Status.ToString(),
                        AccountStatus = (acc.Status == 0 ? "" : _commonFacade.GetConfigAccountStatus(acc.Status).AccountStatusName),
                        Product = pdList?.Product
                    };
                }
                else
                {
        ...
        public ActionResult CustomerContactList(ContactSearchFilter searchFilter)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    //string[] casaType = { Constants.AccountType.CurrentAccount, Constants.AccountType.SavingsAccount };

                    var prodList = (new ProductServiceController()).ProductServiceListWS(new AccountSearchFilter()
                    {
                        CustomerId = searchFilter.CustomerId,
                        CustomerNumber = searchFilter.CustomerNumber
                    });

                    contactVM.ContactList.Where(w => w.AccountNo != null).ToList()
                        .ForEach(a => a.Account = prodList.Where(x => x.AccountNo == a.AccountNo).FirstOrDefault());
                    //.ForEach(a => a.Account = MappingCASAAccountEntity(a.AccountNo, a.CustomerNumber.ToString()));

                    if (TempData["CustomerInfo" + searchFilter.CustomerId.ToString() + searchFilter.CustomerNumber.ToString()] != null)
                    {
                        contactVM.CustomerInfo = (CustomerInfoViewModel)TempData["CustomerInfo" + searchFilter.CustomerId.ToString() + searchFilter.CustomerNumber.ToString()];
                        TempData["CustomerInfo" + searchFilter.CustomerId.ToString() + searchFilter.CustomerNumber.ToString()] = contactVM.CustomerInfo;
                    }
        ...
        [ValidateAntiForgeryToken]
        [CheckUserRole(ScreenCode.ViewCustomerContact)]
        public ActionResult InitEditCustomerContact(int? contactId = null, int? customerId = null, decimal? customerNumber = null)
        {
            ...
            try
            {
                ...
                if (contactId.HasValue && customerId.HasValue)
                {
                    ...
                    contactEditVM.CbsCardTypeId = contactEntity.CbsCardType != null ? contactEntity.CbsCardType.CardTypeID.ConvertToString() : string.Empty;
                    ...
                    contactEditVM.CustomerNumber = customerNumber;
                    ...
                    contactRelationships.Where(w => w.AccountNo != null && casaType.Contains(w.AccountType)).ToList()
                        .ForEach(a => a.Account = prodList.Where(x => x.AccountNo == a.AccountNo).FirstOrDefault());
                        //.ForEach(a => a.Account = MappingCASAAccountEntity(a.AccountNo, customerNumber.ToString()));
                ...
                // Get SelectList
                _commonFacade = new CommonFacade();
                contactEditVM.CbsCardTypeList = new SelectList((IEnumerable)_commonFacade.GetCbsCardTypeSelectList(), "Key", "Value", string.Empty);
        ...
        public ActionResult ManageContactRlat(ContactEditViewModel contactEditVM)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(contactEditVM.CbsCardTypeId))
                {
                    _commonFacade = new CommonFacade();
                    ...
                    else if (contactEditVM.CbsCardTypeId.ToNullable<int>() == cardTypePersonal.CardTypeID)
                    {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    ContactEntity contactEntity = new ContactEntity
                    {
                        ...
                        {
                            CardTypeID = (contactEditVM.CbsCardTypeId != null ? Convert.ToInt32(contactEditVM.CbsCardTypeId) : 0)
                        },
                        ...
                        CustomerNumber = contactEditVM.CustomerNumber
                    };
                    ...
                    else
                    {
                        ...
                        if (contactEditVM.ContactRelationshipList.Count(x => ((contactEditVM.CustomerId ?? 0) == 0 || x.CustomerId == contactEditVM.CustomerId.Value)
                            && ((contactEditVM.CustomerNumber ?? 0) == 0 || x.CustomerNumber == contactEditVM.CustomerNumber.Value)) == 0)
                        {
                            // Alert
                            return Json(new
                            {
        ...
        public ActionResult InitEditContactRelationship(string jsonData)
        {
            ...
            try
            {
                ...
                if (contactRelationVM.ListIndex != null)
                {
                    ...
                    contactRelationVM.AccountId = selected.Account.AccountId;
                    contactRelationVM.AccountNo = selected.AccountNo;
                    contactRelationVM.RelationshipId = selected.RelationshipId;
                    contactRelationVM.Product = selected.Product;
                }

                ViewBag.ErrorMessage = "";
                if (contactRelationVM.CustomerId != null && contactRelationVM.CustomerNumber != null)
                {
                    var listAccount = new List<AccountEntity>();
                    if (contactRelationVM.CustomerId.Value > 0)
                    {
                        listAccount = _customerFacade.GetAccountByCustomerId(contactRelationVM.CustomerId.Value);
                    }
                    else if (contactRelationVM.CustomerNumber.Value > 0)
                    {
                        ProductServiceController pd = new ProductServiceController();
                        ...
                        CBSAccountSummaryResponse cstAcc = pd.InquiryCustomerAccountSummaryWS(contactRelationVM.CustomerNumber.Value, vPaging);
                        if (cstAcc.ResponseCode == "CBS-I-1000" || cstAcc.ResponseCode == "CBS-M-2001")
                        {
                            listAccount = cstAcc.AccountList.ToList();
                        }
                        else {
                            ViewBag.ErrorMessage = cstAcc.ResponseCode + " : " + cstAcc.ResponseMessage;
                        }
                    }
                    ...
                    contactRelationVM.AccountList = gAcct;
                }
        ...
        {
            Logger.Info(_logMsg.Clear().SetPrefixMsg("Edit ContactRelationship").Add("AccountNo", contactRelationVM.AccountNo)
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var objCheckDup =
                        contactRelationVM.ContactRelationshipList.FirstOrDefault(
                            x => x.AccountNo == contactRelationVM.AccountNo);
                    if (objCheckDup != null && custContact.AccountNo != objCheckDup.AccountNo)
                    {
                        ModelState.AddModelError("AccountNo", Resource.Error_SaveDuplicateContact);
                        goto Outer;
                    }

                    if (custContact.Account == null)
                        custContact.Account = new AccountEntity();

                    custContact.Account.AccountId = contactRelationVM.AccountId;
                    custContact.Account.AccountNo = contactRelationVM.AccountNo;
                    custContact.AccountNo = contactRelationVM.AccountNo;
                    custContact.Account.AccountType = contactRelationVM.AccountType;
                    ...
                    if (TempData["AccountList"] != null)
                    {
                        ...
                        var objAccount = lstAccount.FirstOrDefault(x => x.AccountNo == contactRelationVM.AccountNo);
                        custContact.Account = new AccountEntity
                        {
                            AccountId = objAccount.AccountId,
                            AccountNo = objAccount.AccountNo,
                            AccountDesc = objAccount.AccountDesc,
                            AccountType = objAccount.AccountType ?? contactRelationVM.AccountType,
                            Product = objAccount.Product
                    };
                        custContact.AccountNo = objAccount.AccountNo;
                        custContact.Product = objAccount.Product;
                        custContact.CustomerFullNameThaiEng = objAccount.CustomerName;
                    ...
                    if (contactRelationVM.ListIndex == null)
                    {
                        ...
                        custContact.CustomerNumber = contactRelationVM.CustomerNumber;
                        custContact.IsEdit = true;

                        #region "Customer Name"
                        if (custContact.CustomerId.Value > 0)
                        {
        ...
        [CheckUserRole(ScreenCode.ViewCustomerContact)]
        public JsonResult GetProduct(string accountNo)
        {
            if (TempData["AccountList"] != null)
            {
                var lstAccount = (List<AccountEntity>)TempData["AccountList"];
                var objAccount = lstAccount.FirstOrDefault(x => x.AccountNo == accountNo);
