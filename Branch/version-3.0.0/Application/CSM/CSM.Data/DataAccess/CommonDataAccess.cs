...
namespace CSM.Data.DataAccess
{
    public class CommonDataAccess : ICommonDataAccess
    {
        ...
        {
            var query = from t in _context.TB_M_SUBSCRIPT_TYPE
                        where t.ID_TYPE_CODE == IDTypeCode
                        select new SubscriptTypeEntity
                        {
                            SubscriptTypeId = t.SUBSCRIPT_TYPE_ID,
                            SubscriptTypeName = t.SUBSCRIPT_TYPE_NAME,
        ...
        public CasaAccountStatusEntity GetConfigCasaAccountStatus(long casaAccountStatusValue)
        {
            var query = from ac in _context.TB_C_CASA_ACCOUNT_STATUS
                         where ac.CASA_ACCOUNT_STATUS_VALUE == casaAccountStatusValue
                         select new CasaAccountStatusEntity
                         {
                             CasaAccountStatusId = ac.CASA_ACCOUNT_STATUS_ID,
                             CasaAccountStatusValue = ac.CASA_ACCOUNT_STATUS_VALUE,
                             CasaAccountStatusName = ac.CASA_ACCOUNT_STATUS_NAME
                         };

            return (query.Any() ? query.FirstOrDefault() : new CasaAccountStatusEntity());
        }

        public CbsCustomerCategoryEntity GetCbsCustomerCategory(string customerCategoryValue) {
            var query = from cc in _context.TB_C_CUSTOMER_CATEGORY
                        where cc.CUSTOMER_CATEGORY_VALUE == customerCategoryValue
                        select new CbsCustomerCategoryEntity
                        {
            ...
            return (query.Any() ? query.FirstOrDefault() : new CbsCustomerCategoryEntity());
        }
