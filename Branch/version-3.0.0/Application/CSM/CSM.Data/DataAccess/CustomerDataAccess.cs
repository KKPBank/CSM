    ...
    public class CustomerDataAccess : ICustomerDataAccess
    {
        ...
        public IEnumerable<CustomerEntity> SearchCustomer(CustomerSearchFilter searchFilter)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.FirstName))
            {
                ...
                if (!string.IsNullOrEmpty(originalKeyword) && originalKeyword != "*" && originalKeyword != "**")
                {
                    if (originalKeyword.StartsWith("*") && originalKeyword.EndsWith("*"))
                    {
                        ...
                        query = query.Where(q => q.FIRST_NAME_TH.Contains(keyword) || q.FIRST_NAME_EN.Contains(keyword));
                    }
                    else if (originalKeyword.StartsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(1, originalKeyword.Length - 1);
                        query = query.Where(q => q.FIRST_NAME_TH.EndsWith(keyword) || q.FIRST_NAME_EN.EndsWith(keyword));
                    }
                    else if (originalKeyword.EndsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(0, originalKeyword.Length - 1);
                        query = query.Where(q => q.FIRST_NAME_TH.StartsWith(keyword) || q.FIRST_NAME_EN.StartsWith(keyword));
                    }
                    else
                    {
            ...
            if (!string.IsNullOrEmpty(searchFilter.LastName))
            {
                ...
                if (!string.IsNullOrEmpty(originalKeyword) && originalKeyword != "*" && originalKeyword != "**")
                {
                    ...
                    {
                        var keyword = originalKeyword.Substring(1, originalKeyword.Length - 2);
                        query = query.Where(q => q.LAST_NAME_TH.Contains(keyword) || q.LAST_NAME_EN.Contains(keyword));
                    }
                    else if (originalKeyword.StartsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(1, originalKeyword.Length - 1);
                        query = query.Where(q => q.LAST_NAME_TH.EndsWith(keyword) || q.LAST_NAME_EN.EndsWith(keyword));
                    }
                    else if (originalKeyword.EndsWith("*"))
                    {
                        var keyword = originalKeyword.Substring(0, originalKeyword.Length - 1);
                        query = query.Where(q => q.LAST_NAME_TH.StartsWith(keyword) || q.LAST_NAME_EN.StartsWith(keyword));
                    }
                    else
                    {
            ...
            var results = (from c in query
                           ...
                           from ct in _context.TB_C_CUSTOMER_CATEGORY.Where(ct => ct.CUSTOMER_CATEGORY_VALUE == c.TB_M_SUBSCRIPT_TYPE.CUST_TYPE_GROUP).DefaultIfEmpty()
                           select new CustomerEntity
                           {
                               CustomerId = c.CUSTOMER_ID,
                               CustomerNumber = 0,
                               ...
                               LastNameEnglish = c.LAST_NAME_EN,
                               FirstNameThaiEng = !string.IsNullOrEmpty(c.FIRST_NAME_TH) ? c.FIRST_NAME_TH : c.FIRST_NAME_EN,
                               LastNameThaiEng = !string.IsNullOrEmpty(c.LAST_NAME_TH) ? c.LAST_NAME_TH : c.LAST_NAME_EN,
                               CardNo = c.CARD_NO,
                               IDTypeCode = sbt.SUBSCRIPT_TYPE_NAME ?? "",
                               SubscriptType = new SubscriptTypeEntity
                               {
                                   SubscriptTypeId = sbt.SUBSCRIPT_TYPE_ID,
                                   SubscriptTypeCode = sbt.SUBSCRIPT_TYPE_CODE ?? "",
                                   SubscriptTypeName = sbt.SUBSCRIPT_TYPE_NAME ?? ""
                               },
                               BirthDate = c.BIRTH_DATE,
                               CustomerType = c.TYPE ?? 0,
                               CustomerCategory = ct.LONG_DESC_THAI
                          }).AsQueryable();
                    ...
                    try
                    {
                        ...
                        if (customerEntity.CustomerId == null || customerEntity.CustomerId == 0)
                        {
                            ...
                            lstCustomerContact.Add(new CustomerContactEntity
                            {
                                ...
                                Account = new AccountEntity { AccountId = accountDummy },
        ...
        public IEnumerable<AttachmentEntity> GetAttachmentList(AttachmentSearchFilter searchFilter)
        {
            ...
            var querySR = from at in _context.TB_T_SR_ATTACHMENT
                          ...
                          && (cust.SUBSCRIPT_TYPE_NAME == searchFilter.SubsTypeName && cust.CARD_NO == searchFilter.CardNo)
                          select new AttachmentEntity
                          {
        ...
        public bool SaveContactSr(ContactEntity contactEntity)
        {
            ...
            try
            {
                ...
                // AddCustomerLog
                this.AddCustomerLog(contactEntity.CustomerId.Value, contactEntity.CustomerNumber.Value, Constants.CustomerLog.AddContact, contactEntity.UpdateUser.UserId);
        ...
        private void SaveContactRelationship(List<CustomerContactEntity> lstCustomerContact, int contactId, int updateUser)
        {
            foreach (CustomerContactEntity customerContactEntity in lstCustomerContact)
            {
                ...
                if (isIDNull)
                {
                    ...
                    if (customerContactEntity.CustomerNumber.Value != 0)
        ...
        public IEnumerable<SrEntity> GetSrList(SrSearchFilter searchFilter)
        {
            ...
            var query = from sr in _context.TB_T_SR
                        ...
                        where (isNotFilterOwner == 1 || sr.SR_STATUS_ID == Constants.SRStatusId.Draft ||
        ...
        public IEnumerable<SrEntity> GetCustomerSrList(SrSearchFilter searchFilter)
        {
            ...
            var query = from sr in _context.TB_T_SR
                        ...
                        where (cust.SUBSCRIPT_TYPE_NAME == searchFilter.SubscripTypeName && cust.CARD_NO == searchFilter.CardNo)
                        select new SrEntity
                        {
                            SrId = sr.SR_ID,
                            RuleAlertNo = sr.RULE_THIS_ALERT,
