...
import com.motiftech.icollection.RuleEngineAssign;
import com.motiftech.icollection.RuleEmailAssign;
...
public class BatchRuleEmailAssign extends AbstractBatch{
	...
	public void init(){
		try{
          ...
          if(workTime != null){
              String[] times = StringUtils.split(workTime.getOptionDesc()!=null?workTime.getOptionDesc():InquiryConstants.Time.END_TIME,":");
              log.info("workTime [HOUR:MIN] : " + workTime.getOptionDesc());
...
//              }
          ...
          log.info("END-TIME      : " + END_CURRENT_TIME);
          log.info("END-TIMEUNIT  : " + END_TIMEUNIT);
          log.info("END-ASSIGNED  : " + calToEndTime.getTime());
		}catch(RuntimeException ex){
			log.debug("init(SMS) : "+ex);
			init();
		}
	}
	...
	public void run(String... args){
		     try{
	            init();
	            final RuleEmailAssign runner               = new RuleEmailAssign(args);
	            ...
	            log.error("run BatchRuleAssign", ex);
	        }
	}

	public static void main(String[] args) {
		System.out.println("=========== START Call BatchRuleEmailAssign ============");
		BatchRuleEmailAssign test = new BatchRuleEmailAssign();
		test.run(args);
		System.out.println("=========== END Call BatchRuleEmailAssign ============");
	}
