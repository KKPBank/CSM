...
namespace CSM.Data.DataAccess
{
    public class CustomerDataAccess : ICustomerDataAccess
    {
        ...
        #region "Customer"
        public IEnumerable<CustomerEntity> OutsourceGetCustomerList(CustomerSearchFilter searchFilter)
        {
        ...
        public IEnumerable<CustomerEntity> GetCustomerList(CustomerSearchFilter searchFilter)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.AccountNo))
            {
                query = query.Where(x => x.ACCOUNT_NO.ToUpper().Equals(searchFilter.AccountNo.ToUpper()));
            }

            // Filter Product
            if (!string.IsNullOrEmpty(searchFilter.Product))
            {
                query = query.Where(x => x.PRODUCT.Equals(searchFilter.Product));
            }

            // Filter Grade
            if (!string.IsNullOrEmpty(searchFilter.Grade))
            {
                query = query.Where(x => x.GRADE.Equals(searchFilter.Grade));
            }

            // Filter BranchName
            if (!string.IsNullOrEmpty(searchFilter.BranchName))
            {
                query = query.Where(x => x.BRANCH_NAME.Equals(searchFilter.BranchName));
            }
            ...
            if (status != null && status != Constants.ApplicationStatus.All)
            {
                if (status == 1)
                {
                    query = query.Where(x => x.STATUS == Constants.AccountStatus.Active);
                }
                else if (status == 0)
                {
                    query = query.Where(x => x.STATUS != Constants.AccountStatus.Active);
            ...
            var result = (from c in query
                          ...
                          {
                              CustomerId = c.CUSTOMER_ID,
                              FirstNameThai = c.TB_M_CUSTOMER.FIRST_NAME_TH,
                              FirstNameEnglish = c.TB_M_CUSTOMER.FIRST_NAME_EN,
                              LastNameThai = c.TB_M_CUSTOMER.LAST_NAME_TH,
                              LastNameEnglish = c.TB_M_CUSTOMER.LAST_NAME_EN,
                              FirstNameThaiEng = !string.IsNullOrEmpty(c.TB_M_CUSTOMER.FIRST_NAME_TH) ? c.TB_M_CUSTOMER.FIRST_NAME_TH : c.TB_M_CUSTOMER.FIRST_NAME_EN,
                              LastNameThaiEng = !string.IsNullOrEmpty(c.TB_M_CUSTOMER.FIRST_NAME_TH) ? c.TB_M_CUSTOMER.LAST_NAME_TH : c.TB_M_CUSTOMER.LAST_NAME_EN,
                              CardNo = c.TB_M_CUSTOMER.CARD_NO,
                              ...
                              CustomerType = c.TB_M_CUSTOMER.TYPE,
                              SubscriptType = new SubscriptTypeEntity
                              {
                                  SubscriptTypeId = c.TB_M_CUSTOMER.SUBSCRIPT_TYPE_ID,
                                  SubscriptTypeName = c.TB_M_CUSTOMER.TB_M_SUBSCRIPT_TYPE.SUBSCRIPT_TYPE_NAME,
                              },
            ...
            int startPageIndex = (searchFilter.PageNo - 1) * searchFilter.PageSize;
            searchFilter.TotalRecords = result.Count();
            if (startPageIndex >= searchFilter.TotalRecords)
            {
                startPageIndex = 0;
                searchFilter.PageNo = 1;
            ...
            result = SetCustomerListSort(result, searchFilter);

            var tmp = result.Skip(startPageIndex).Take(searchFilter.PageSize).ToList();

            foreach (var item in tmp)
            {
                item.StrPhoneNo = string.Join(",", item.PhoneList.Select(x => x.PhoneNo).ToList());
            }

            return result.ToList();
        }

        public IEnumerable<CustomerEntity> xxxGetCustomerList(CustomerSearchFilter searchFilter)
        {
