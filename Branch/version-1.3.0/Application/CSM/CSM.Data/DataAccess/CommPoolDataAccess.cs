...
namespace CSM.Data.DataAccess
{
    public class CommPoolDataAccess : ICommPoolDataAccess
    {
        ...
        public List<PoolEmailTemplateEntity> GetPoolEmailTemplates()
        {
            return (from ch in _context.TB_C_SR_EMAIL_TEMPLATE
                    orderby ch.SR_EMAIL_TEMPLATE_NAME
                    select new PoolEmailTemplateEntity
                    {
                        PoolEmailTemplateId = ch.SR_EMAIL_TEMPLATE_ID,
                ...
                using (var transaction = _context.Database.BeginTransaction(IsolationLevel.ReadCommitted))
                {
                    ...
                    try
                    {
                        ...
                        if (poolEntity.PoolId == null || poolEntity.PoolId == 0)
                        {
                            ...
                            pool.REPLY_MAIL = poolEntity.ReplyMail;
                            pool.EMAIL_TEMPLATE_ID = poolEntity.EmailTemplateId;
                        ...
                        if (poolBranches != null && poolBranches.Count > 0)
                        {
        ...
        public PoolEntity GetPoolByID(int commPoolId)
        {
            var query = from p in _context.TB_M_POOL
                        ...
                        select new PoolEntity
                        {
                            ...
                            Email = p.EMAIL,
                            Status = p.STATUS,
                            ReplyMail = p.REPLY_MAIL,
                            EmailTemplateId = p.EMAIL_TEMPLATE_ID,
                            ...
                            foreach (var jobAttach in jobAttachments)
                            {
                                ...
                                if (fileInfo != null && fileInfo.Length > 0)
                                {
                                    if (StreamDataHelpers.TryToCopy(jobFilePath, srFilePath))
                                    {
                                        ...
                                        //add attachment doc type others
                                        List<AttachmentTypeEntity> listATE = new List<AttachmentTypeEntity>();
                                        AttachmentTypeEntity ate = new AttachmentTypeEntity();
                                        ate.AttachmentId = _context.TB_T_SR_ATTACHMENT_DOCUMENT_TYPE.Max(x => x.SR_ATTACHMENT_DOCUMENT_TYPE_ID) + 1;
                                        ate.DocTypeId = 48; //doc type others.
                                        ate.Name = _context.TB_M_DOCUMENT_TYPE.FirstOrDefault(i => i.DOCUMENT_TYPE_ID == ate.DocTypeId).DOCUMENT_TYPE_NAME;
                                        ate.CreateUserId = userId;
                                        listATE.Add(ate);
