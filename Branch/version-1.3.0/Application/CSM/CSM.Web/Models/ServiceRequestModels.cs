...
namespace CSM.Web.Models
{
    ...
    public class CreateServiceRequestViewModel
    {
        ...
        public CreateServiceRequestViewModel(ServiceRequestForDisplayEntity sr)
        {
            CanEdit = true;
            IsClone = true;

            //assign mores for subject.
            Subject = sr.Subject;
            Remark = sr.Remark;

            CustomerBirthDate = sr.Customer.BirthDateDisplay;
            CustomerCardNo = sr.Customer.CardNo;
            CustomerContactId = sr.Contact.CustomerContactId;
            CustomerEmail = sr.Customer.Email;
            CustomerEmployeeCode = sr.CustomerEmployeeCode;
            CustomerFax = sr.Customer.Fax;
            CustomerFirstNameEn = sr.Customer.FirstNameEnglish;
            CustomerFirstNameTh = sr.Customer.FirstNameThai;
            CustomerId = sr.Customer.CustomerId;
            CustomerLastNameEn = sr.Customer.LastNameEnglish;
            CustomerLastNameTh = sr.Customer.LastNameThai;
            if(sr.Customer.PhoneList.Count > 0) {
                CustomerPhoneNo1 = sr.Customer.PhoneList[0].PhoneNo;
                CustomerPhoneTypeName1 = sr.Customer.PhoneList[0].PhoneTypeName;
            }
            if (sr.Customer.PhoneList.Count > 1) {
                CustomerPhoneNo2 = sr.Customer.PhoneList[1].PhoneNo;
                CustomerPhoneTypeName2 = sr.Customer.PhoneList[1].PhoneTypeName;
            }
            if (sr.Customer.PhoneList.Count > 2) {
                CustomerPhoneNo3 = sr.Customer.PhoneList[2].PhoneNo;
                CustomerPhoneTypeName3 = sr.Customer.PhoneList[2].PhoneTypeName;
            }
            if (sr.CustomerSubscriptType != null) CustomerSubscriptionTypeCode = sr.CustomerSubscriptType.SubscriptTypeCode;
            
            if (sr.Customer.TitleEnglish != null) { CustomerTitleEn = sr.Customer.TitleEnglish.TitleName; }
            if (sr.Customer.TitleThai != null) { CustomerTitleTh = sr.Customer.TitleThai.TitleName; }

            //assign more properties for Contact Profile.
            AccountNo = sr.Account.AccountNo;
            if (sr.ContactSubscriptType != null) ContactSubscriptionName = sr.ContactSubscriptType.SubscriptTypeName;
            AccountId = sr.Account.AccountId;

            ContactAccountNo = sr.ContactAccountNo;
            ContactBirthDate = sr.Contact.BirthDateDisplay;
            ContactCardNo = sr.Contact.CardNo;
            ContactEmail = sr.Contact.Email;
            ContactFax = sr.Contact.Fax;
            ContactFirstNameEn = sr.Contact.FirstNameEnglish;
            ContactFirstNameTh = sr.Contact.FirstNameThai;
            ContactId = sr.Contact.ContactId;
            ContactLastNameEn = sr.Contact.LastNameEnglish;
            ContactLastNameTh = sr.Contact.LastNameThai;
            if (sr.Customer.PhoneList.Count > 0)
            {
                ContactPhoneNo1 = sr.Contact.PhoneList[0].PhoneNo;
                ContactPhoneTypeName1 = sr.Contact.PhoneList[0].PhoneTypeName;
            }
            if (sr.Customer.PhoneList.Count > 1)
            {
                ContactPhoneNo2 = sr.Contact.PhoneList[1].PhoneNo;
                ContactPhoneTypeName2 = sr.Contact.PhoneList[1].PhoneTypeName;
            }
            if (sr.Customer.PhoneList.Count > 2)
            {
                ContactPhoneNo3 = sr.Contact.PhoneList[2].PhoneNo;
                ContactPhoneTypeName3 = sr.Contact.PhoneList[2].PhoneTypeName;
            }
            //ContactRelationshipId = sr.Contact.RelationshipId;
            //ContactRelationshipName = sr.Contact.RelationshipNameDisplay;
            ContactRelationshipId = sr.Relationship.RelationshipId;
            ContactRelationshipName = sr.Relationship.RelationshipName;
            if (sr.Contact.TitleEnglish != null) { ContactTitleEn = sr.Contact.TitleEnglish.TitleName; }
            if (sr.Contact.TitleThai != null) { ContactTitleTh = sr.Contact.TitleThai.TitleName; }

            if (sr.CampaignService != null)
            {
                CampaignServiceId = sr.CampaignService.CampaignServiceId;
                CampaignServiceName = sr.CampaignService.CampaignServiceName;
            }

            if (sr.Channel != null)
            {
                ChannelId = sr.Channel.ChannelId;
                ChannelName = sr.Channel.Name;
            }

            if (sr.ProductGroup != null)
            {
                ProductGroupId = sr.ProductGroup.ProductGroupId;
                ProductGroupName = sr.ProductGroup.ProductGroupName;
            }

            if (sr.Product != null)
            {
                ProductId = sr.Product.ProductId;
                ProductName = sr.Product.ProductName;
            }

            if (sr.Area != null)
            {
                AreaId = sr.Area.AreaId;
                AreaName = sr.Area.AreaName;
            }

            if (sr.SubArea != null)
            {
                SubAreaId = sr.SubArea.SubareaId;
                SubAreaName = sr.SubArea.SubareaName;
            }

            if (sr.Type != null)
            {
                TypeId = sr.Type.TypeId;
                TypeName = sr.Type.TypeName;
            }
            if (sr.MediaSource != null)
            {
                MediaSourceId = sr.MediaSource.MediaSourceId;
                MediaSourceName = sr.MediaSource.Name;
            }

            if (sr.SRStatus != null)
            {
                SRStatusId = sr.SRStatus.SRStatusId;
                SRStatusName = sr.SRStatus.SRStatusName;
            }

            if (sr.CloseDate != null)
            {
                CloseDate = sr.CloseDate;
            }
            IsClose = sr.DraftIsClose;
        }
