...
import com.motiftech.icollection.process.CancelSRProcess;
import com.motiftech.icollection.service.AccountService;
import com.utils.ApplicationContextHolder;

public class RuleCancelSR implements Runnable{

	private static final Logger log = Logger.getLogger(RuleCancelSR.class);
	String[] args;
	
	public static void main(String[] args){
		RuleCancelSR rule = new RuleCancelSR(args);
		rule.run();
	}
	
	public RuleCancelSR(String[] args) {	
		this.args = args;
	}
	...
	public void run() {
		try{
			...
			long accountCount 				= accountService.countSRByAssignFlag(InquiryConstants.AssignFlag.ASSIGN);
			int threadCount 				= 1;
			if(args.length>0){
				try{
			...
			for(int i=0;i<threadCount;i++){
				...
				runners[i] = new CancelSRProcess(startIndex,fetchSize);
				runners[i].execute();
