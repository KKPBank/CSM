    ...
    public class ServiceRequestDataAccess : IServiceRequestDataAccess
    {
        ...
        public IEnumerable<ServiceRequestEntity> SearchServiceRequest(ServiceRequestSearchFilter searchFilter)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.PhoneNo))
            {
                ...
                query = query.Where(q => q.TB_M_CUSTOMER.TB_M_PHONE.Any(p => p.PHONE_NO.Contains(searchFilter.PhoneNo)));
            }
        ...
        public void CreateSrAttachmentDocumentType(List<SrAttachmentEntity> attachments, int? createUserId)
        {

            foreach (var attachment in attachments)
            {
                TB_T_SR_ATTACHMENT_DOCUMENT_TYPE attach_doc_type = new TB_T_SR_ATTACHMENT_DOCUMENT_TYPE();
                attach_doc_type.SR_ATTACHMENT_ID = (int)attachment.SrAttachId;
                attach_doc_type.SR_ATTACHMENT_DOCUMENT_TYPE_ID = 24;
                attach_doc_type.CREATE_USER = createUserId;
                attach_doc_type.CREATE_DATE = DateTime.Now;

                _context.TB_T_SR_ATTACHMENT_DOCUMENT_TYPE.Add(attach_doc_type);
            }
        ...
        //public ServiceRequestSaveSrReportResult SaveServiceRequestReport(ServiceRequestForSaveEntity entity, TB_T_SR sr, bool isSaveDraft)
        //{
        ...
        //        TB_R_SR_REPORT sr_rpt;

        //        if (!isEdit)
        //        {
        //            sr_rpt = new TB_R_SR_REPORT();
        //        }
        //        else
        //        {
        //            sr_rpt = _context.TB_R_SR_REPORT.SingleOrDefault(s => s.SR_NO == entity.SrId.Value.ToString());

        //            if (sr_rpt == null)
        //            {
        //                return new ServiceRequestSaveSrReportResult(false, string.Format("SR_ID: {0} does not exist in database.", entity.SrId.Value));
        //            }
        //        }

        //        sr_rpt.SR_NO = string.IsNullOrEmpty(entity.SrNo) ? null : entity.SrNo;

        //        if (sr.OWNER_BRANCH_ID.HasValue && sr != null)
        //            CalculateTotalSLA(sr.OWNER_BRANCH_ID.Value, sr, new TB_L_SR_LOGGING(), DateTime.Now);

        //        sr_rpt.TOTAL_SLA = sr.RULE_TOTAL_ALERT;
        //        sr_rpt.CURRENT_ALERT = sr.RULE_THIS_ALERT;

        //        TB_M_CUSTOMER cs = _context.TB_M_CUSTOMER.SingleOrDefault(s => s.CUSTOMER_ID == sr.CUSTOMER_ID);
        //        if (cs != null) {
        //            sr_rpt.FIRST_NAME_TH = cs.FIRST_NAME_TH;
        //            sr_rpt.FIRST_NAME_EN = cs.FIRST_NAME_EN;
        //            sr_rpt.LAST_NAME_TH = cs.LAST_NAME_TH;
        //            sr_rpt.LAST_NAME_EN = cs.LAST_NAME_EN;
        //            sr_rpt.CARD_NO = cs.CARD_NO;
        //        }
                
        //        TB_M_ACCOUNT ac = _context.TB_M_ACCOUNT.SingleOrDefault(s => s.ACCOUNT_ID == sr.ACCOUNT_ID);
        //        if (ac != null) {
        //            sr_rpt.ACCOUNT_NO = ac.ACCOUNT_NO;
        //            sr_rpt.CAR_REGIS_NO = ac.CAR_NO;
        //        }

        //        TB_R_BRANCH br = _context.TB_R_BRANCH.SingleOrDefault(s => s.BRANCH_ID == sr.CREATE_BRANCH_ID);
        //        if (br != null) sr_rpt.CREATOR_BRANCH = br.BRANCH_NAME;

        //        TB_R_CHANNEL ch = _context.TB_R_CHANNEL.SingleOrDefault(s => s.CHANNEL_ID == sr.CHANNEL_ID);
        //        if (ch != null){
        //            sr_rpt.CHANNEL_NAME = ch.CHANNEL_NAME;
        //            sr_rpt.JOB_TYPE = ch.CHANNEL_NAME;
        //        }

        //        sr_rpt.CALL_ID = sr.SR_CALL_ID;
        //        sr_rpt.ANO = sr.SR_ANO;

        //        TB_R_PRODUCTGROUP pg = _context.TB_R_PRODUCTGROUP.SingleOrDefault(s => s.PRODUCTGROUP_ID == sr.PRODUCTGROUP_ID);
        //        if (pg != null) sr_rpt.PRODUCTGROUP_NAME = pg.PRODUCTGROUP_NAME;

        //        TB_R_PRODUCT pd = _context.TB_R_PRODUCT.SingleOrDefault(s => s.PRODUCT_ID == sr.PRODUCT_ID);
        //        if (pd != null) sr_rpt.PRODUCT_NAME = pd.PRODUCT_NAME;

        //        TB_R_CAMPAIGNSERVICE cp = _context.TB_R_CAMPAIGNSERVICE.SingleOrDefault(s => s.CAMPAIGNSERVICE_ID == sr.CAMPAIGNSERVICE_ID);
        //        if (cp != null) sr_rpt.CAMPAIGNSERVICE_NAME = cp.CAMPAIGNSERVICE_NAME;

        //        TB_M_TYPE ty = _context.TB_M_TYPE.SingleOrDefault(s => s.TYPE_ID == sr.TYPE_ID);
        //        if (ty != null) sr_rpt.TYPE_NAME = ty.TYPE_NAME;

        //        TB_M_AREA ar = _context.TB_M_AREA.SingleOrDefault(s => s.AREA_ID == sr.AREA_ID);
        //        if (ar != null) sr_rpt.AREA_NAME = ar.AREA_NAME;

        //        TB_M_SUBAREA su = _context.TB_M_SUBAREA.SingleOrDefault(s => s.SUBAREA_ID == sr.SUBAREA_ID);
        //        if (su != null) sr_rpt.SUBAREA_NAME = su.SUBAREA_NAME;

        //        TB_C_SR_STATUS st = _context.TB_C_SR_STATUS.SingleOrDefault(s => s.SR_STATUS_ID == sr.SR_STATUS_ID);
        //        if (st != null) sr_rpt.SR_STATUS_NAME = st.SR_STATUS_NAME;

        //        sr_rpt.CLOSE_DATE = (DateTime?)entity.CloseDate;
        //        sr_rpt.SR_IS_VERIFY_PASS = FormatVerifyPass(entity.IsVerifyPass);

        //        TB_R_USER us = _context.TB_R_USER.SingleOrDefault(s => s.USER_ID == sr.CREATE_USER);
        //        if (us != null) {
        //            sr_rpt.SR_STATUS_NAME = st.SR_STATUS_NAME;
        //            sr_rpt.CREATOR_NAME = us.POSITION_CODE + '-' + us.FIRST_NAME + ' ' + us.LAST_NAME;
        //            sr_rpt.ROLE_ID = us.ROLE_ID;
        //        }
                    
        //        sr_rpt.CREATE_DATE = now;

        //        us = _context.TB_R_USER.SingleOrDefault(s => s.USER_ID == sr.OWNER_USER_ID);
        //        sr_rpt.OWNER_NAME = us != null?us.POSITION_CODE + '-' + us.FIRST_NAME + ' ' + us.LAST_NAME:null;

        //        sr_rpt.UPDATE_DATE_BY_OWNER = sr.UPDATE_DATE_BY_OWNER;

        //        us = _context.TB_R_USER.SingleOrDefault(s => s.USER_ID == sr.DELEGATE_USER_ID);
        //        if(us!=null) sr_rpt.DELEGATOR_NAME = us.POSITION_CODE + '-' + us.FIRST_NAME + ' ' + us.LAST_NAME;

        //        sr_rpt.UPDATE_DATE_BY_DELEGATE = sr.UPDATE_DATE_BY_DELEGATE;

        //        //sr_rpt.SR_SUBJECT = entity.Subject ?? string.Empty;
        //        //sr_rpt.SR_REMARK = entity.Remark ?? string.Empty;

        //        TB_M_CONTACT ct = _context.TB_M_CONTACT.SingleOrDefault(s => s.CONTACT_ID == sr.CONTACT_ID);
        //        if (ct != null)
        //        {
        //            sr_rpt.CONTACT_NAME = ct.FIRST_NAME_TH;
        //            sr_rpt.CONTACT_SURNAME = ct.LAST_NAME_TH;
        //        }

        //        TB_M_RELATIONSHIP rl = _context.TB_M_RELATIONSHIP.SingleOrDefault(s => s.RELATIONSHIP_ID == sr.CONTACT_RELATIONSHIP_ID);
        //        if (rl != null) sr_rpt.RELATIONSHIP = rl.RELATIONSHIP_NAME;

        //        TB_M_MEDIA_SOURCE ms = _context.TB_M_MEDIA_SOURCE.SingleOrDefault(s => s.MEDIA_SOURCE_ID == sr.MEDIA_SOURCE_ID);
        //        if (ms != null) sr_rpt.MEDIA_SOURCE_NAME = ms.MEDIA_SOURCE_NAME;

        //        TB_T_SR_ACTIVITY sa = _context.TB_T_SR_ACTIVITY.SingleOrDefault(s => s.SR_ID == sr.SR_ID);
        //        TB_T_SR_ATTACHMENT sm;
        //        if (sa != null) sm = _context.TB_T_SR_ATTACHMENT.SingleOrDefault(s => s.SR_ACTIVITY_ID == sa.SR_ACTIVITY_ID);
        //        sr_rpt.ATTACH_FILE = "Yes";//sm.SR_ATTACHMENT_ID

        //        TB_M_CONTACT_PHONE co = _context.TB_M_CONTACT_PHONE.SingleOrDefault(s => s.CONTACT_ID == sr.CONTACT_ID);
        //        if (co != null) sr_rpt.CONTACT_NO = co.PHONE_NO;

        //        sr_rpt.PHONE_TYPE_CODE = "02";
        //        sr_rpt.SR_PAGE_ID = entity.SrPageId;

        //        sr_rpt.OWNER_USER_ID = entity.OwnerUserId;
        //        sr_rpt.OWNER_BRANCH_ID = entity.OwnerBranchId;

        //        sr_rpt.PRODUCTGROUP_ID = entity.ProductGroupId;
        //        sr_rpt.PRODUCT_ID = entity.ProductId;
        //        sr_rpt.CAMPAIGNSERVICE_ID = entity.CampaignServiceId;
        //        sr_rpt.AREA_ID = entity.AreaId;
        //        sr_rpt.SUBAREA_ID = entity.SubAreaId;
        //        sr_rpt.TYPE_ID = entity.TypeId;
        //        // Set value for DRAFT or clear when OPEN
        //        sr_rpt.SR_STATUS_ID = entity.SrStatusId;
        //        sr_rpt.CHANNEL_ID = entity.ChannelId;

        //        if (!isEdit)
        //        {
        //            // Save
        //            sr_rpt.CREATE_BRANCH_ID = entity.CreateBranchId;
        //            sr_rpt.CREATE_USER = entity.CreateUserId;
        //            _context.TB_R_SR_REPORT.Add(sr_rpt);
        //        }
        //        else
        //        {
        //            SetEntryStateModified(sr_rpt);
        //        }

        //        _context.SaveChanges();

        //        entity.SrId = int.Parse(sr_rpt.SR_NO);
        //        entity.CreateDate = now;

        //        return new ServiceRequestSaveSrReportResult(true, "", entity.SrId, "", sr_rpt.SR_NO);
        //    }
        //    catch (Exception ex)
        //    {
        //        Logger.Error("Exception occur:\n", ex);
        //        return new ServiceRequestSaveSrReportResult(false, ex.Message);
        //    }
        //    finally
        //    {
        ...
        public ServiceRequestSaveSrResult SaveServiceRequest(ServiceRequestForSaveEntity entity, bool isSaveDraft)
        {
            ...
            try
            {
                ...
                sr.SR_SUBJECT = !string.IsNullOrEmpty(entity.Subject) ?
                    ApplicationHelpers.StripHTML(entity.Subject.Replace(",", string.Empty))
                    : string.Empty;
                sr.SR_REMARK = !string.IsNullOrEmpty(entity.Remark) ?
                    ApplicationHelpers.StripHTML(entity.Remark.Replace(",",string.Empty))
                    : string.Empty;
                ...
                if (entity.IsClone) {
                    _context.Entry(sr).State = System.Data.Entity.EntityState.Added;
                }
                Save();

                //call [asynchronus] save TB_R_SR_REPORT for csv report sr.
                //SaveServiceRequestReport(entity, sr, isSaveDraft);
        ...
        public void UpdateServiceRequest(ServiceRequestForSaveEntity entity)
        {
            ...
            {
                if (entity == null)
                    throw new Exception("Technical Error: Argument is null");

                if (!entity.SrId.HasValue || entity.SrId.Value == 0)
                    throw new Exception("Technical Error: Sr Id is null or 0");
                ...
                //call [asynchronus] save TB_R_SR_REPORT for csv report sr.
                //SaveServiceRequestReport(entity, sr, false );//isSaveDraft
            }
            finally
            {
                _context.Configuration.AutoDetectChangesEnabled = true;
        ...
        public void CopyServiceRequest(ServiceRequestForSaveEntity entity)
        {
            _context.Configuration.AutoDetectChangesEnabled = false;
            ...
            try
            {
                ...
                TB_T_SR sr = _context.TB_T_SR.SingleOrDefault(x => x.SR_ID == entity.SrId.Value);

                sr.SR_ID = _context.TB_T_SR.Max(x=>x.SR_ID) + 1;
                sr.SR_NO = entity.SrNo;
                sr.SR_STATUS_ID = entity.SrStatusId;
                sr.CREATE_USER = entity.CreateUserId;
                sr.CREATE_DATE = entity.CreateDate;
        ...
        public void UpdateServiceRequest(ServiceRequestNoDetailEntity newValue, DateTime now)
        {
            ...
            try
            {
                ...
                {
                    if (newValue.SrStatusId == Constants.SRStatusId.Closed)
                    {
        ...
        public void UpdateSubmitCARStatusToActivity(int srActivityId, short status, string errorCode, string errorMessage)
        {
            ...
            try
            {
                ...
                if (activity == null)
                    throw new Exception("Technical Error: Not found SR Activity in database. (TB_T_SR_ACTIVITY)");
        ...
        private void CalculateTotalSLA(int branchId, TB_T_SR sr, TB_L_SR_LOGGING logging, DateTime createdDate)
        {
            try
            {
                    ...
                    || !branch.END_TIME_MINUTE.HasValue)
                {
                    ...
                    if (start != null)
                    {
                        ...
                        if (str.Count() == 2 && str[0].Trim() != "" && str[1].Trim() != "")
                        {
                            ...
                            throw new Exception("Invalid: " + branch.BRANCH_NAME);
                    }
                    else
                        throw new Exception("Invalid: " + branch.BRANCH_NAME);
                ...
                else
                {
                    ...
                    endTimeMin = Convert.ToInt32(branch.START_TIME_MINUTE ?? 0);
        ...
        public IEnumerable<ServiceRequestContactSearchResult> SearchContact(CustomerContactSearchFilter searchFilter)
        {
            ...
            if (!string.IsNullOrEmpty(searchFilter.PhoneNo))
            {
                ...
                query = query.Where(q => q.TB_M_CONTACT.TB_M_CONTACT_PHONE.Any(p => p.PHONE_NO.Contains(searchFilter.PhoneNo)));
            }
        ...
        public IQueryable<ServiceRequestActivityResult> GetSrActivityQueryable(bool isSrOnly, int? srId, int? customerId)
        {
            return (from sa in _context.TB_T_SR_ACTIVITY
                    ...
                    select new ServiceRequestActivityResult
                    {
                        ...
                        OfficerUserFirstName = office != null ? office.FIRST_NAME : string.Empty,
