...
namespace CSM.Data.DataAccess
{
    public class ReportDataAccess : IReportDataAccess
    {
        ...
        //Common Export SR Method with SR Report Table.
        /*public IList<ExportSREntity> GetExportSR(ExportSRSearchFilter searchFilter)
        {
            ...
            #endregion
            //List<ExportSREntity> exportSRs;

            var query = (from sr in _context.TB_R_SR_REPORT.AsNoTracking()
                         from s in _context.TB_T_SR.Where(s => s.SR_NO == sr.SR_NO).DefaultIfEmpty()
                         where
                            (sr.SR_PAGE_ID == Constants.SRPage.DefaultPageId || sr.SR_PAGE_ID == Constants.SRPage.AFSPageId || !sr.SR_PAGE_ID.HasValue ||
                            (sr.ROLE_ID.HasValue && lstRole.Contains(sr.ROLE_ID.Value)))
                         ...
                         && (string.IsNullOrEmpty(searchFilter.FirstName) || (sr.FIRST_NAME_TH.Contains(searchFilter.FirstName) || sr.FIRST_NAME_EN.ToUpper().Contains(searchFilter.FirstName.ToUpper())))
                         && (string.IsNullOrEmpty(searchFilter.LastName) || (sr.LAST_NAME_TH.Contains(searchFilter.LastName) || sr.LAST_NAME_TH.ToUpper().Contains(searchFilter.LastName.ToUpper())))
                         && (string.IsNullOrEmpty(searchFilter.CardId) || sr.CARD_NO.Contains(searchFilter.CardId))
                         && (productGroupId == null || sr.PRODUCTGROUP_ID == productGroupId)
                         && (productId == null || sr.PRODUCT_ID == productId)
                         && (campaignId == null || sr.CAMPAIGNSERVICE_ID == campaignId)
                         && (typeId == null || sr.TYPE_ID == typeId)
                         && (areaId == null || sr.AREA_ID == areaId)
                         && (subareaId == null || sr.SUBAREA_ID == subareaId)
                         && (slaId == null || slaId == Constants.ApplicationStatus.All
                                    || (slaId == 1 && (sr.CURRENT_ALERT == null || sr.CURRENT_ALERT == 0))
                                    || (slaId == 2 && (sr.CURRENT_ALERT.HasValue && sr.CURRENT_ALERT > 0))
                               )
                         && (srstatusId == null || srstatusId == -1 || sr.SR_STATUS_ID == srstatusId)
                         && (channelId == null || channelId == -1 || sr.CHANNEL_ID == channelId)
                         ...
                         && (string.IsNullOrEmpty(searchFilter.AccountNo) || sr.ACCOUNT_NO == searchFilter.AccountNo)
                         select new ExportSREntity
                         {
                             TotalSla = sr.TOTAL_SLA, //sr.RULE_TOTAL_WORK,
                             CurrentAlert = sr.CURRENT_ALERT,
                             CustomerFistname = !string.IsNullOrEmpty(sr.FIRST_NAME_TH) ? sr.FIRST_NAME_TH : sr.FIRST_NAME_EN,
                             CustomerLastname = !string.IsNullOrEmpty(sr.FIRST_NAME_TH) ? sr.LAST_NAME_TH : sr.LAST_NAME_EN,
                             CardNo = sr.CARD_NO,
                             AccountNo = sr.ACCOUNT_NO,
                             CarRegisNo = sr.CAR_REGIS_NO,
                             SRNo = sr.SR_NO,
                             CreatorBranch = sr.CREATOR_BRANCH,
                             ChannelName = sr.CHANNEL_NAME,
                             CallId = sr.CALL_ID,
                             ANo = sr.ANO,
                             ...
                             CreatorName = sr.CREATOR_NAME,
                             CreateDate = sr.CREATE_DATE,
                             OwnerName = sr.OWNER_NAME,
                             UpdateDateOwner = sr.UPDATE_DATE_BY_OWNER,
                             DelegatorName = sr.DELEGATOR_NAME,
                             UpdateDelegate = sr.UPDATE_DATE_BY_DELEGATE,
                             //SRSubject = sr.SR_SUBJECT,
                             //SRRemark = sr.SR_REMARK,
                             SRSubject = s.SR_SUBJECT,
                             SRRemark = s.SR_REMARK,             
                             ContactName = sr.CONTACT_NAME,
                             ContactSurname = sr.CONTACT_SURNAME,
                             Relationship = sr.RELATIONSHIP,
                             ContactNo = sr.CONTACT_NO,
                             MediaSourceName = sr.MEDIA_SOURCE_NAME
                             //AttachFile = sr.ATTACH_FILE,
                             //JobType = sr.JOB_TYPE,
                         });

            return query.ToList();

            //return new List<ExportSREntity>();
            //return exportSRs;
        }*/

        //Monthly Export SR Method with SR Report Table.
        ...
        /* Get Report SR with Stored Procedure solution.
        public IList<ExportSREntity> GetExportSR(ExportSRSearchFilter searchFilter)
        {
            DateTime? srMinDate = null;
            DateTime? srMaxDate = null;
            ...
            string cs = ConfigurationManager.ConnectionStrings["CSMContext"].ConnectionString;
            EntityConnection ec = new EntityConnection(cs);
            //IDbConnection myConnection = ec.StoreConnection;
            SqlConnection myConnection = (SqlConnection)ec.StoreConnection;
            //SqlConnection myConnection = new SqlConnection(cs);
            SqlCommand myCommand = new SqlCommand("SP_GET_EXPORT_SR", myConnection);
            List<ExportSREntity> exportSRs;
            ...
            try
            {
                ...
                myCommand.Parameters.Add("@SR_ID", SqlDbType.NVarChar, 50).Value = (object)searchFilter.SRId ?? DBNull.Value;
                //myCommand.Parameters.Add("@ID", SqlDbType.Int
                //myCommand.Parameters["@ID"].Direction = ParameterDirection.Output;
                myCommand.Parameters.Add("@productGroupId", SqlDbType.Int).Value = (object)searchFilter.ProductGroup ?? DBNull.Value;
                myCommand.Parameters.Add("@productId", SqlDbType.Int).Value = (object)searchFilter.Product ?? DBNull.Value;
                myCommand.Parameters.Add("@campaignId", SqlDbType.Int).Value = (object)searchFilter.Campaign ?? DBNull.Value;
                myCommand.Parameters.Add("@typeId", SqlDbType.Int).Value = (object)searchFilter.Type ?? DBNull.Value;
                myCommand.Parameters.Add("@areaId", SqlDbType.Int).Value = (object)searchFilter.Area ?? DBNull.Value;
                myCommand.Parameters.Add("@subareaId", SqlDbType.Int).Value = (object)searchFilter.SubArea ?? DBNull.Value;
                myCommand.Parameters.Add("@slaId", SqlDbType.Int).Value = (object)searchFilter.Sla ?? DBNull.Value;
                myCommand.Parameters.Add("@srstatusId", SqlDbType.Int).Value = (object)searchFilter.SRStatus ?? DBNull.Value;
                myCommand.Parameters.Add("@channelId", SqlDbType.Int).Value = (object)searchFilter.SRChannel ?? DBNull.Value;
                myCommand.Parameters.Add("@ownerId", SqlDbType.Int).Value = (object)searchFilter.OwnerSR ?? DBNull.Value;
                myCommand.Parameters.Add("@ownerBranchId", SqlDbType.Int).Value = (object)searchFilter.OwnerBranch ?? DBNull.Value;
                myCommand.Parameters.Add("@creatorId", SqlDbType.Int).Value = (object)searchFilter.CreatorSR ?? DBNull.Value;
                myCommand.Parameters.Add("@creatorBranchId", SqlDbType.Int).Value = (object)searchFilter.CreatorBranch ?? DBNull.Value;
                myCommand.Parameters.Add("@srMinDate", SqlDbType.DateTime).Value = srMinDate;
                myCommand.Parameters.Add("@srMaxDate", SqlDbType.DateTime).Value = srMaxDate;
                myCommand.Parameters.Add("@firstName", SqlDbType.NVarChar,255).Value = (object)searchFilter.FirstName ?? DBNull.Value;
                myCommand.Parameters.Add("@lastName", SqlDbType.NVarChar,255).Value = (object)searchFilter.LastName ?? DBNull.Value;
                myCommand.Parameters.Add("@cardNo", SqlDbType.NVarChar,50).Value = (object)searchFilter.CardId ?? DBNull.Value;
                myCommand.Parameters.Add("@subject", SqlDbType.NVarChar,255).Value = (object)searchFilter.Subject ?? DBNull.Value;
                myCommand.Parameters.Add("@description", SqlDbType.NVarChar,255).Value = (object)searchFilter.Description ?? DBNull.Value;
                myCommand.Parameters.Add("@accountNo", SqlDbType.NVarChar,255).Value = (object)searchFilter.AccountNo ?? DBNull.Value;

                using (SqlDataReader myDataReader = myCommand.ExecuteReader())
                {
                    exportSRs = new List<ExportSREntity>();

                    while (myDataReader.Read())
                    {
                        ExportSREntity newItem = new ExportSREntity();

                        newItem.TotalSla = myDataReader.IsDBNull(0) ? (int?)null : myDataReader.GetInt32(0);
                        newItem.CurrentAlert = myDataReader.IsDBNull(1) ? (int?)null : myDataReader.GetInt32(1);
                        newItem.CustomerFistname = myDataReader.IsDBNull(2) ? null : myDataReader.GetString(2);
                        newItem.CustomerLastname = myDataReader.IsDBNull(3) ? null : myDataReader.GetString(3);
                        newItem.CardNo = myDataReader.IsDBNull(4) ? null : myDataReader.GetString(4);
                        newItem.AccountNo = myDataReader.IsDBNull(5) ? null : myDataReader.GetString(5);
                        newItem.CarRegisNo = myDataReader.IsDBNull(6) ? null : myDataReader.GetString(6);
                        newItem.SRNo = myDataReader.IsDBNull(7) ? null : myDataReader.GetString(7);
                        newItem.CreatorBranch = myDataReader.IsDBNull(8) ? null : myDataReader.GetString(8);
                        newItem.ChannelName = myDataReader.IsDBNull(9) ? null : myDataReader.GetString(9);
                        newItem.CallId = myDataReader.IsDBNull(10) ? null : myDataReader.GetString(10);
                        newItem.ANo = myDataReader.IsDBNull(11) ? null : myDataReader.GetString(11);
                        newItem.ProductGroupName = myDataReader.IsDBNull(12) ? null : myDataReader.GetString(12);
                        newItem.ProductName = myDataReader.IsDBNull(13) ? null : myDataReader.GetString(13);
                        newItem.CampaignServiceName = myDataReader.IsDBNull(14) ? null : myDataReader.GetString(14);
                        newItem.TypeName = myDataReader.IsDBNull(15) ? null : myDataReader.GetString(15);
                        newItem.AreaName = myDataReader.IsDBNull(16) ? null : myDataReader.GetString(16);
                        newItem.SubAreaName = myDataReader.IsDBNull(17) ? null : myDataReader.GetString(17);
                        newItem.SRStatusName = myDataReader.IsDBNull(18) ? null : myDataReader.GetString(18);
                        newItem.CloseDate = myDataReader.IsDBNull(19) ? (DateTime?)null : myDataReader.GetDateTime(19);
                        newItem.SRIsverifyPass = myDataReader.IsDBNull(20) ? null : myDataReader.GetString(20);
                        newItem.CreatorName = myDataReader.IsDBNull(21) ? null : myDataReader.GetString(21);
                        newItem.CreateDate = myDataReader.IsDBNull(22) ? (DateTime?)null : myDataReader.GetDateTime(22);
                        newItem.OwnerName = myDataReader.IsDBNull(23) ? null : myDataReader.GetString(23);
                        newItem.UpdateDateOwner = myDataReader.IsDBNull(24) ? (DateTime?)null : myDataReader.GetDateTime(24);
                        newItem.DelegatorName = myDataReader.IsDBNull(25) ? null : myDataReader.GetString(25);
                        newItem.UpdateDelegate = myDataReader.IsDBNull(26) ? (DateTime?)null : myDataReader.GetDateTime(26);
                        newItem.SRSubject = myDataReader.IsDBNull(27) ? null : myDataReader.GetString(27);
                        newItem.SRRemark = myDataReader.IsDBNull(28) ? null : myDataReader.GetString(28);
                        newItem.ContactName = myDataReader.IsDBNull(29) ? null : myDataReader.GetString(29);
                        newItem.ContactSurname = myDataReader.IsDBNull(30) ? null : myDataReader.GetString(30);
                        newItem.Relationship = myDataReader.IsDBNull(31) ? null : myDataReader.GetString(31);
                        newItem.MediaSourceName = myDataReader.IsDBNull(32) ? null : myDataReader.GetString(32);
                        newItem.AttachFile = myDataReader.IsDBNull(33) ? null : myDataReader.GetString(33);
                        newItem.JobType = myDataReader.IsDBNull(34) ? null : myDataReader.GetString(34);
                        newItem.ContactNo = myDataReader.IsDBNull(35) ? null : myDataReader.GetString(35);

                        exportSRs.Add(newItem);
                    }
                }
            }
            finally
